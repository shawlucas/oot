...
s32 BgCheck_PosErrorCheck(Vec3f* pos, char* file, s32 line) {
    if (pos->x >= BGCHECK_XYZ_ABSMAX || pos->x <= -BGCHECK_XYZ_ABSMAX || pos->y >= BGCHECK_XYZ_ABSMAX ||
        ...
        // "Position is invalid."
        osSyncPrintf("BgCheck_PosErrorCheck():position not valid. pos(%f,%f,%f) file:%s line:%d\n", pos->x,
...
void DynaSSNodeList_SetSSListHead(DynaSSNodeList* nodeList, SSList* ssList, s16* polyId) {
    ...
    ASSERT(newNodeId != SS_NULL, "newNodeId != SS_NULL", "../z_bgcheck.c", 1776);
    SSNode_SetValue(&nodeList->tbl[newNodeId], polyId, ssList->head);
    ssList->head = newNodeId;
}
...
void DynaSSNodeList_Alloc(PlayState* play, DynaSSNodeList* nodeList, s32 max) {
    ...
    ASSERT(nodeList->tbl != NULL, "nodeList->tbl != NULL", "../z_bgcheck.c", 1811);
...
void CollisionPoly_GetVerticesByBgId(CollisionPoly* poly, s32 bgId, CollisionContext* colCtx, Vec3f* dest) {
    ...
    if (poly == NULL || bgId > BG_ACTOR_MAX || dest == NULL) {
        ...
        osSyncPrintf("CollisionPoly_GetVerticesByBgId(): Error %d %d %d argument not appropriate. Processing terminated\n",
...
s32 BgCheck_SphVsStaticWall(StaticLookup* lookup, CollisionContext* colCtx, u16 xpFlags, f32* outX, f32* outZ,
    ...
    while (true) {
        ...
        ASSERT(!IS_ZERO(normalXZ), "!IS_ZERO(normalXZ)", "../z_bgcheck.c", 2854);
    ...
    while (true) {
        ...
        ASSERT(!IS_ZERO(normalXZ), "!IS_ZERO(normalXZ)", "../z_bgcheck.c", 2964);
...
void BgCheck_Allocate(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader) {
    ...
    if ((R_SCENE_CAM_TYPE == SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) ||
        ...
        if (play->sceneId == SCENE_STABLE) {
            ...
            osSyncPrintf("/* BGCheck LonLon Size %d byte */\n", 0x3520);
            colCtx->memSize = 0x3520;
        } else {
            // "/* BGCheck Mini Size %d byte */\n"
            osSyncPrintf("/* BGCheck Mini Size %d byte */\n", 0x4E20);
        ...
        osSyncPrintf("/* BGCheck Spot Size %d byte */\n", 0xF000);
        ...
        osSyncPrintf("/* BGCheck Normal Size %d byte  */\n", colCtx->memSize);
        colCtx->dyna.polyNodesMax = 1000;
        colCtx->dyna.polyListMax = 512;
        colCtx->dyna.vtxListMax = 512;
    ...
    if (colCtx->lookupTbl == NULL) {
        LogUtils_HungupThread("../z_bgcheck.c", __LINE__);
    }
    ...
    osSyncPrintf("/*---final BG size %d byte---*/\n", memSize + lookupTblMemSize);
    osSyncPrintf(VT_RST);
...
CollisionHeader* BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId) {
    ...
    if (!(colCtx->dyna.bgActorFlags[bgId] & BGACTOR_IN_USE)) {
        ...
        osSyncPrintf("BgCheck_GetCollisionHeader():that bg_actor_index isn't used. index =%d\n");
        osSyncPrintf(VT_RST);
        return NULL;
    }
...
f32 BgCheck_RaycastDownImpl(PlayState* play, CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly,
    ...
    *outPoly = NULL;
    ...
    while (true) {
        ...
        if (BgCheck_PosErrorCheck(&checkPos, "../z_bgcheck.c", __LINE__)) {
            if (actor != NULL) {
                osSyncPrintf("this one, actor->id %d\n", actor->id);
            }
        }
...
s32 BgCheck_CheckWallImpl(CollisionContext* colCtx, u16 xpFlags, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev,
    ...
    *posResult = *posNext;
    ...
    if (BgCheck_PosErrorCheck(posNext, "../z_bgcheck.c", __LINE__) == true ||
        BgCheck_PosErrorCheck(posPrev, "../z_bgcheck.c", __LINE__) == true) {
        if (actor != NULL) {
            osSyncPrintf("this one,actor->id %d\n", actor->id);
        }
    }
...
s32 BgCheck_CheckCeilingImpl(CollisionContext* colCtx, u16 xpFlags, f32* outY, Vec3f* pos, f32 checkHeight,
    ...
    *outBgId = BGCHECK_SCENE;
    *outY = pos->y;
    if (BgCheck_PosErrorCheck(pos, "../z_bgcheck.c", __LINE__) == true) {
        if (actor != NULL) {
            osSyncPrintf("this one,actor->id %d\n", actor->id);
        }
    }
...
s32 BgCheck_CheckLineImpl(CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2, Vec3f* posA, Vec3f* posB,
    ...
    *outBgId = BGCHECK_SCENE;
    if (BgCheck_PosErrorCheck(posA, "../z_bgcheck.c", __LINE__) == true ||
        BgCheck_PosErrorCheck(posB, "../z_bgcheck.c", __LINE__) == true) {
        if (actor != NULL) {
            osSyncPrintf("this one,actor->id %d\n", actor->id);
        } else {
            osSyncPrintf("actor == NULL, offender unclear\n");
        }
    }
...
void SSNodeList_Alloc(PlayState* play, SSNodeList* this, s32 tblMax, s32 numPolys) {
    ...
    ASSERT(this->tbl != NULL, "this->tbl != NULL", "../z_bgcheck.c", __LINE__);

    this->polyCheckTbl = GameState_Alloc(&play->state, numPolys, "../z_bgcheck.c", __LINE__);

    ASSERT(this->polyCheckTbl != NULL, "this->polyCheckTbl != NULL", "../z_bgcheck.c", 5981);
}
...
SSNode* SSNodeList_GetNextNode(SSNodeList* this) {
    ...
    this->count++;
    ASSERT(this->count < this->max, "this->count < this->max", "../z_bgcheck.c",
           5998);
...
u16 SSNodeList_GetNextNodeIdx(SSNodeList* this) {
    ...
    ASSERT(new_index < this->max, "new_index < this->max", "../z_bgcheck.c", 6021);
    return new_index;
}
...
void DynaPoly_AllocPolyList(PlayState* play, CollisionPoly** polyList, s32 numPolys) {
    *polyList = THA_AllocTailAlign(&play->state.tha, numPolys * sizeof(CollisionPoly), ALIGNOF_MASK(CollisionPoly));
    ASSERT(*polyList != NULL, "*polyList != NULL", "../z_bgcheck.c", 6247);
}
...
void DynaPoly_AllocVtxList(PlayState* play, Vec3s** vtxList, s32 numVtx) {
    *vtxList = THA_AllocTailAlign(&play->state.tha, numVtx * sizeof(Vec3s), ALIGNOF_MASK(Vec3s));
    ASSERT(*vtxList != NULL, "*vtxList != NULL", "../z_bgcheck.c", 6277);
}
...
s32 DynaPoly_SetBgActor(PlayState* play, DynaCollisionContext* dyna, Actor* actor, CollisionHeader* colHeader) {
    ...
    if (!foundSlot) {
        osSyncPrintf(VT_FGCOL(RED));
        osSyncPrintf("DynaPoly_SetBgActor():dynamic polygon - no empty indexes\n");
        osSyncPrintf(VT_RST);
        return BG_ACTOR_MAX;
    }
    ...
    osSyncPrintf(VT_FGCOL(GREEN));
    osSyncPrintf("DynaPoly_SetBgActor():index %d\n", bgId);
    osSyncPrintf(VT_RST);
    return bgId;
}
...
void DynaPoly_DeleteBgActor(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    ...
    osSyncPrintf(VT_FGCOL(GREEN));
    osSyncPrintf("DynaPoly_DeleteBgActor():index %d\n", bgId);
    osSyncPrintf(VT_RST);
    if (!DynaPoly_IsBgIdBgActor(bgId)) {
        ...
        if (bgId == -1) {
            ...
            osSyncPrintf(
                "DynaPoly_DeleteBgActor(): should have been deleted (?)\ndue to index (== -1) process will be stopped\n");
            osSyncPrintf(VT_RST);
            return;
        } else {
            osSyncPrintf(VT_FGCOL(RED));
            // "Unable to deallocate index / index unallocated, processing aborted."
            osSyncPrintf("DynaPoly_DeleteBgActor():"
                         "due to the deallocation of unreserved/impossible-to-reserve indexes, process will be stopped. index == %d\n",
                         bgId);
            osSyncPrintf(VT_RST);
            return;
        }
...
void DynaPoly_AddBgActorToLookup(PlayState* play, DynaCollisionContext* dyna, s32 bgId, s32* vtxStartIndex,
    ...
    if (!(dyna->polyListMax >= *polyStartIndex + pbgdata->numPolygons)) {
        ...
        osSyncPrintf("DynaPoly_AddBgActorToLookup():polygons exceeded - %d cannot be over %d\n",
    ...
    if (!(dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices)) {
        osSyncPrintf(VT_FGCOL(RED));
        // "do not use if %d exceeds %d"
        osSyncPrintf("DynaPoly_AddBgActorToLookup():vertices exceeded - %d cannot be over %d\n",
                     *vtxStartIndex + pbgdata->numVertices, dyna->vtxListMax);
    }

    ASSERT(dyna->polyListMax >= *polyStartIndex + pbgdata->numPolygons,
           "dyna->polyListMax >= *polyStartIndex + pbgdata->numPolygons", "../z_bgcheck.c", 6687);
    ASSERT(dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices,
           "dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices", "../z_bgcheck.c", 6688);
...
void DynaPoly_UpdateContext(PlayState* play, DynaCollisionContext* dyna) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_1) {
            // Initialize BgActor
            osSyncPrintf(VT_FGCOL(GREEN));
            osSyncPrintf("DynaPoly_UpdateContext(): delete index=%d\n", i);
            osSyncPrintf(VT_RST);
...
s32 BgCheck_SphVsDynaWallInBgActor(CollisionContext* colCtx, u16 xpFlags, DynaCollisionContext* dyna, SSList* ssList,
    ...
    while (true) {
        ...
        CollisionPoly_GetNormalF(poly, &nx, &ny, &nz);
        normalXZ = sqrtf(SQ(nx) + SQ(nz));
        ASSERT(!IS_ZERO(normalXZ), "!IS_ZERO(normalXZ)", "../z_bgcheck.c", 7382);
    ...
    while (true) {
        ...
        CollisionPoly_GetNormalF(poly, &nx, &ny, &nz);
        normalXZ = sqrtf(SQ(nx) + SQ(nz));
        ASSERT(!IS_ZERO(normalXZ), "!IS_ZERO(normalXZ)", "../z_bgcheck.c", 7489);
