...
s32 BgCheck_PosErrorCheck(Vec3f* pos, const char* file, int line) {
    if (pos->x >= BGCHECK_XYZ_ABSMAX || pos->x <= -BGCHECK_XYZ_ABSMAX || pos->y >= BGCHECK_XYZ_ABSMAX ||
        ...
        PRINTF(VT_FGCOL(RED));
        // "Position is invalid."
        PRINTF("BgCheck_PosErrorCheck(): Position is invalid pos (%f,%f,%f) file:%s line:%d\n", pos->x, pos->y,
               pos->z, file, line);
        PRINTF(VT_RST);
        return true;
    }
...
void CollisionPoly_GetVerticesByBgId(CollisionPoly* poly, s32 bgId, CollisionContext* colCtx, Vec3f* dest) {
    ...
    if (poly == NULL || bgId > BG_ACTOR_MAX || dest == NULL) {
        ...
        PRINTF("CollisionPoly_GetVerticesByBgId(): Error %d %d %d Argument not appropriate. Processing terminated \n",
...
void BgCheck_Allocate(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader) {
    ...
    PRINTF("/*---------------- BGCheck Buffer Memory Size -------------*/\n");
    ...
    if ((R_SCENE_CAM_TYPE == SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) ||
        ...
        if (play->sceneId == SCENE_STABLE) {
            ...
            PRINTF("/* BGCheck LonLon Size %dbyte */\n", 0x3520);
            colCtx->memSize = 0x3520;
        } else {
            // "/* BGCheck Mini Size %dbyte */\n"
            PRINTF("/* BGCheck Mini Size %dbyte */\n", 0x4E20);
        ...
        PRINTF("/* BGCheck Spot Size %dbyte */\n", 0xF000);
        ...
        PRINTF("/* BGCheck Normal Size %dbyte  */\n", colCtx->memSize);
        colCtx->dyna.polyNodesMax = 1000;
        colCtx->dyna.polyListMax = 512;
        colCtx->dyna.vtxListMax = 512;
    ...
    PRINTF("/*---BG size used in the end %dbyte---*/\n", memSize + lookupTblMemSize);
    PRINTF(VT_RST);
...
CollisionHeader* BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId) {
    ...
    if (!(colCtx->dyna.bgActorFlags[bgId] & BGACTOR_IN_USE)) {
        ...
        PRINTF("BgCheck_GetCollisionHeader:That bgId is not used. index=%d\n");
        PRINTF(VT_RST);
        return NULL;
    }
    ...
    *outBgId = BGCHECK_SCENE;
    ...
    if (BgCheck_PosErrorCheck(posA, "../z_bgcheck.c", 5334) == true ||
        ...
        if (actor != NULL) {
            ...
            PRINTF("actor == NULL offender unclear\n");
        }
    }
#endif
...
s32 DynaPoly_SetBgActor(PlayState* play, DynaCollisionContext* dyna, Actor* actor, CollisionHeader* colHeader) {
    ...
    if (!foundSlot) {
        PRINTF(VT_FGCOL(RED));
        PRINTF("DynaPoly_SetBgActor(): dynamic polygon - no empty indexes\n");
        PRINTF(VT_RST);
        return BG_ACTOR_MAX;
    }
    ...
    PRINTF(VT_FGCOL(GREEN));
    PRINTF("DynaPoly_SetBgActor():bgId %d\n", bgId);
    PRINTF(VT_RST);
    return bgId;
}
...
void DynaPoly_DeleteBgActor(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    ...
    PRINTF(VT_FGCOL(GREEN));
    PRINTF("DynaPoly_DeleteBgActor():index %d\n", bgId);
    PRINTF(VT_RST);
    if (!DynaPoly_IsBgIdBgActor(bgId)) {
        ...
        if (bgId == -1) {
            ...
            PRINTF("DynaPoly_DeleteBgActor():The index that should have been deleted(? ) was(== -1), processing aborted.\n");
            PRINTF(VT_RST);
        } else {
            PRINTF(VT_FGCOL(RED));
            // "Unable to deallocate index / index unallocated, processing aborted."
            PRINTF("DynaPoly_DeleteBgActor():"
...
void DynaPoly_AddBgActorToLookup(PlayState* play, DynaCollisionContext* dyna, s32 bgId, s32* vtxStartIndex,
    ...
    if (!(dyna->polyListMax >= *polyStartIndex + pbgdata->numPolygons)) {
        ...
        PRINTF("DynaPoly_AddBgActorToLookup():polygons exceeded - %d cannot be over %d\n", *polyStartIndex + pbgdata->numPolygons,
               dyna->polyListMax);
    }
    ...
    if (!(dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices)) {
        PRINTF(VT_FGCOL(RED));
        // "do not use if %d exceeds %d"
        PRINTF("DynaPoly_AddBgActorToLookup():vertices exceeded - %d cannot be over %d\n", *vtxStartIndex + pbgdata->numVertices,
               dyna->vtxListMax);
    }
    ...
    ASSERT(dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices,
           "dyna->vtxListMax >= *vtxStartIndex + pbgdata->numVertices", "../z_bgcheck.c", __LINE__);
#endif
...
void DynaPoly_UpdateContext(PlayState* play, DynaCollisionContext* dyna) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_1) {
            // Initialize BgActor
            PRINTF(VT_FGCOL(GREEN));
            PRINTF("DynaPoly_UpdateContext():initialize bgactor index=%d\n", i);
            PRINTF(VT_RST);
        ...
        if (dyna->bgActors[i].actor != NULL && dyna->bgActors[i].actor->update == NULL) {
            // Delete BgActor
            PRINTF(VT_FGCOL(GREEN));
            PRINTF("DynaPoly_UpdateContext():delete bgActor index=%d\n", i);
            PRINTF(VT_RST);
