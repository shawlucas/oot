...
void PadMgr_ProcessInputs(PadMgr* padmgr) {
    ...
    for (i = 0; i < padmgr->ncontrollers; i++, input++, padnow1++) {
        ...
        input->press.x += (s8)(input->cur.x - input->prev.x);
        input->press.y += (s8)(input->cur.y - input->prev.y);
    }
    ...
    for (i = 0; i < 4; i++) {
        if (mode != 0) {
            *newInput = *ogInput;
            ...
            newInput->press.x += (s8)(newInput->cur.x - newInput->prev.x);
            newInput->press.y += (s8)(newInput->cur.y - newInput->prev.y);
        }
        ogInput++;
        newInput++;
    }
