...
void Skin_Init(PlayState* play, Skin* skin, SkeletonHeader* skeletonHeader, AnimationHeader* animationHeader) {
    ...
    skin->vtxTable = ZeldaArena_MallocDebug(limbCount * sizeof(SkinLimbVtx), "../z_skin_awb.c", __LINE__);

    ASSERT(skin->vtxTable != NULL, "skin->vtxTable != NULL", "../z_skin_awb.c", __LINE__);
    ...
    for (i = 0; i < limbCount; i++) {
        ...
        if ((limb->segmentType != SKIN_LIMB_TYPE_ANIMATED) || (limb->segment == NULL)) {
            ...
            vtxEntry->buf[0] =
                ZeldaArena_MallocDebug(animatedLimbData->totalVtxCount * sizeof(Vtx), "../z_skin_awb.c", __LINE__);
            ASSERT(vtxEntry->buf[0] != NULL, "vtxEntry->buf[0] != NULL", "../z_skin_awb.c", __LINE__);

            vtxEntry->buf[1] =
                ZeldaArena_MallocDebug(animatedLimbData->totalVtxCount * sizeof(Vtx), "../z_skin_awb.c", __LINE__);
            ASSERT(vtxEntry->buf[1] != NULL, "vtxEntry->buf[1] != NULL", "../z_skin_awb.c", __LINE__);
    ...
    if (skin->vtxTable != NULL) {
        ...
        for (i = 0; i < skin->limbCount; i++) {
            if (skin->vtxTable[i].buf[0] != NULL) {
                ZeldaArena_FreeDebug(skin->vtxTable[i].buf[0], "../z_skin_awb.c", __LINE__);
                skin->vtxTable[i].buf[0] = NULL;
            }
            if (skin->vtxTable[i].buf[1] != NULL) {
                ZeldaArena_FreeDebug(skin->vtxTable[i].buf[1], "../z_skin_awb.c", __LINE__);
                skin->vtxTable[i].buf[1] = NULL;
            }
        }

        if (skin->vtxTable != NULL) {
            ZeldaArena_FreeDebug(skin->vtxTable, "../z_skin_awb.c", __LINE__);
        }
