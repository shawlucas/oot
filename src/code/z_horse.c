...
void func_8006D074(GlobalContext* globalCtx) {
    gSaveContext.memory.horseData.scene = SCENE_SPOT00;
    gSaveContext.memory.horseData.pos.x = -1840;
    gSaveContext.memory.horseData.pos.y = 72;
    gSaveContext.memory.horseData.pos.z = 5497;
    gSaveContext.memory.horseData.angle = -27353;
}

void func_8006D0AC(GlobalContext* globalCtx) {
    if (gSaveContext.memory.horseData.scene == SCENE_SPOT06) {
        gSaveContext.memory.horseData.scene = SCENE_SPOT06;
        gSaveContext.memory.horseData.pos.x = -2065;
        gSaveContext.memory.horseData.pos.y = -863;
        gSaveContext.memory.horseData.pos.z = 1839;
        gSaveContext.memory.horseData.angle = 0;
    }
}
...
void func_8006D0EC(GlobalContext* globalCtx, Player* player) {
    ...
    if ((AREG(6) != 0) && (Flags_GetEventChkInf(0x18) || (DREG(1) != 0))) {
        ...
        gSaveContext.memory.horseData.scene = globalCtx->sceneNum;
        ...
        horseActor->room = -1;
    } else if ((gSaveContext.entranceIndex == 1230) && (gSaveContext.memory.information.eventChkInf[1] & 0x100)) {
        Actor* horseActor;
        horseActor =
    ...
    } else if ((globalCtx->sceneNum == gSaveContext.memory.horseData.scene) &&
               (Flags_GetEventChkInf(0x18) != 0 || DREG(1) != 0)) {
        // Translates to: "SET BY EXISTENCE OF HORSE %d %d %d"
        osSyncPrintf("馬存在によるセット %d %d %d\n", gSaveContext.memory.horseData.scene, Flags_GetEventChkInf(0x18),
                     DREG(1));

        if (func_8006CFC0(gSaveContext.memory.horseData.scene)) {
            Actor* horseActor;
            horseActor = Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_HORSE, gSaveContext.memory.horseData.pos.x,
                                     gSaveContext.memory.horseData.pos.y, gSaveContext.memory.horseData.pos.z, 0,
                                     gSaveContext.memory.horseData.angle, 0, 1);
            if (horseActor == NULL) {
