...
#undef DEFINE_GAMESTATE_INTERNAL
    ...
    LOG_ADDRESS("gameStateInitFunc", gameStateInitFunc, "../graph.c", 696);
    return NULL;
}
...
void Graph_Update(GraphicsContext* gfxCtx, GameState* gameState) {
    ...
    if (gIsCtrlr2Valid && PreNmiBuff_IsResetting(gAppNmiBufferPtr) && !gameState->inPreNMIState) {
        // "To reset mode"
        PRINTF(VT_COL(YELLOW, BLACK) "PRE-Nmi to reset mode\n" VT_RST);
...
void* Graph_Alloc2(GraphicsContext* gfxCtx, size_t size) {
    ...
    if (HREG(59) == 1) {
        PRINTF("Graph_Alloc2 siz=%d thga size=%08x bufp=%08x head=%08x tail=%08x\n", size, thga->size, thga->start,
               thga->p, thga->d);
    }
    return THGA_AllocTail(&gfxCtx->polyOpa, ALIGN16(size));
}
