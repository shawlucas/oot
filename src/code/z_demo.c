    ...
    switch (cmd->base) {
        ...
        case 16:
            if (sp3F != 0) {
                ...
                Quake_SetSpeed(D_8015FCCA, SHT_MAX);
...
void Cutscene_ProcessCommands(GlobalContext* globalCtx, CutsceneContext* csCtx, u8* cutscenePtr) {
    ...
    MemCopy(&totalEntries, cutscenePtr, sizeof(s32));
    cutscenePtr += 4;
    MemCopy(&cutsceneEndFrame, cutscenePtr, sizeof(s32));
    cutscenePtr += 4;
    ...
    for (i = 0; i < totalEntries; i++) {
        MemCopy(&cmdType, cutscenePtr, sizeof(s32));
        cutscenePtr += 4;
        ...
        switch (cmdType) {
            case CS_CMD_MISC:
                MemCopy(&cmdEntries, cutscenePtr, sizeof(s32));
                cutscenePtr += 4;
...
void func_80068C3C(GlobalContext* globalCtx, CutsceneContext* csCtx) {
    ...
    if (gSaveContext.cutsceneIndex >= 0xFFF0) {
        ...
        if (BREG(0) != 0) {
            ...
            displayList = Graph_GfxPlusOne(prevDisplayList = POLY_OPA_DISP);
