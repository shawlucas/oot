...
void Map_SetPaletteData(GlobalContext* globalCtx, s16 room) {
    ...
    osSyncPrintf("ＰＡＬＥＴＥセット 【 i=%x : room=%x 】Room_Inf[%d][4]=%x  ( map_palete_no = %d )\n", paletteNum,
                 room, mapIndex, gSaveContext.memory.information.sceneFlags[mapIndex].rooms, interfaceCtx->mapPaletteNum);
    osSyncPrintf(VT_RST);
...
void Map_SetFloorPalettesData(GlobalContext* globalCtx, s16 floor) {
    ...
    if (gSaveContext.memory.table.dungeonItems[mapIndex] & gBitFlags[DUNGEON_MAP]) {
    ...
    switch (globalCtx->sceneNum) {
        ...
        case SCENE_GANON_TOU:
            ...
            if (globalCtx->sceneNum == SCENE_SPOT02) {
                ...
                if ((LINK_AGE_IN_YEARS == YEARS_ADULT) && !((gSaveContext.memory.information.eventChkInf[9] & 0xF) == 0xF)) {
...
void Minimap_DrawCompassIcons(GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_map_exp.c", 565);

    if (globalCtx->interfaceCtx.minimapAlpha >= 0xAA) {
        func_80094A14(globalCtx->state.gfxCtx);

        gSPMatrix(NEXT_OVERLAY_DISP, &gMtxClear, G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        ...
        gDPSetCombineMode(NEXT_OVERLAY_DISP, G_CC_PRIMITIVE, G_CC_PRIMITIVE);
        ...
        gSPMatrix(NEXT_OVERLAY_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_map_exp.c", 585),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

        gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, 200, 255, 0, 255);
        gSPDisplayList(NEXT_OVERLAY_DISP, D_0400C820);
        ...
        gSPMatrix(NEXT_OVERLAY_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_map_exp.c", 603),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

        gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0xFF, 200, 0, 0, 255);
        gSPDisplayList(NEXT_OVERLAY_DISP, D_0400C820);
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_map_exp.c", 607);
}
#else
...
void Minimap_Draw(GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_map_exp.c", 626);
        ...
        switch (globalCtx->sceneNum) {
            ...
            case SCENE_ICE_DOUKUTO:
                if (!R_MINIMAP_TOGGLED) {
                    ...
                    gDPSetCombineLERP(NEXT_OVERLAY_DISP, 1, 0, PRIMITIVE, 0, TEXEL0, 0, PRIMITIVE, 0, 1, 0, PRIMITIVE,
                                      0, TEXEL0, 0, PRIMITIVE, 0);

                    if (gSaveContext.memory.table.dungeonItems[mapIndex] & gBitFlags[DUNGEON_MAP]) {
                        gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, 100, 255, 255, interfaceCtx->minimapAlpha);

                        gDPLoadTextureBlock_4b(NEXT_OVERLAY_DISP, interfaceCtx->mapSegment, G_IM_FMT_I, 96, 85, 0,
                                               G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                                               G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

                        gSPTextureRectangle(NEXT_OVERLAY_DISP, R_DGN_MINIMAP_X << 2, R_DGN_MINIMAP_Y << 2,
                                            (R_DGN_MINIMAP_X + 96) << 2, (R_DGN_MINIMAP_Y + 85) << 2, G_TX_RENDERTILE,
                                            0, 0, 1024, 1024);
                    }

                    if (gSaveContext.memory.table.dungeonItems[mapIndex] & gBitFlags[DUNGEON_COMPASS]) {
            ...
            case SCENE_GANON_TOU:
                if (!R_MINIMAP_TOGGLED) {
                    ...
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, R_MINIMAP_COLOR(0), R_MINIMAP_COLOR(1),
                                    R_MINIMAP_COLOR(2), interfaceCtx->minimapAlpha);

                    gDPLoadTextureBlock_4b(NEXT_OVERLAY_DISP, interfaceCtx->mapSegment, G_IM_FMT_IA,
                                           gMapData->owMinimapWidth[mapIndex], gMapData->owMinimapHeight[mapIndex], 0,
                                           G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                                           G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

                    gSPTextureRectangle(NEXT_OVERLAY_DISP, R_OW_MINIMAP_X << 2, R_OW_MINIMAP_Y << 2,
                    ...
                    if (((globalCtx->sceneNum != SCENE_SPOT01) && (globalCtx->sceneNum != SCENE_SPOT04) &&
                        ...
                        if ((gMapData->owEntranceFlag[sEntranceIconMapIndex] == 0xFFFF) ||
                            ((gMapData->owEntranceFlag[sEntranceIconMapIndex] != 0xFFFF) &&
                             (gSaveContext.memory.information.infTable[26] & gBitFlags[gMapData->owEntranceFlag[mapIndex]]))) {

                            gDPLoadTextureBlock(NEXT_OVERLAY_DISP, D_02002500, G_IM_FMT_RGBA, G_IM_SIZ_16b, 8, 8, 0,
                                                G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                                                G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
                    ...
                    if ((globalCtx->sceneNum == SCENE_SPOT08) && (gSaveContext.memory.information.infTable[26] & gBitFlags[9])) {
                        gDPLoadTextureBlock(NEXT_OVERLAY_DISP, D_02002500, G_IM_FMT_RGBA, G_IM_SIZ_16b, 8, 8, 0,
                                            G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK,
                                            G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

                        gSPTextureRectangle(NEXT_OVERLAY_DISP, 1080, 616, 1112, 648, G_TX_RENDERTILE, 0, 0, 1024,
                                            1024);
                    }
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_map_exp.c", 782);
}
