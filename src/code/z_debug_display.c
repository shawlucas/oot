...
void DebugDisplay_DrawSpriteI8(DebugDispObject* dispObj, u32 texture, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_debug_display.c", 169);

    func_80094678(globalCtx->state.gfxCtx);

    gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, dispObj->color.r, dispObj->color.g, dispObj->color.b, dispObj->color.a);
    ...
    gDPLoadTextureBlock(NEXT_POLY_XLU_DISP, texture, G_IM_FMT_I, G_IM_SIZ_8b, 16, 16, 0, G_TX_NOMIRROR | G_TX_WRAP,
                        G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

    gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_debug_display.c", 189),
              G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(NEXT_POLY_XLU_DISP, &D_04004298);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_debug_display.c", 192);
}

void DebugDisplay_DrawPolygon(DebugDispObject* dispObj, u32 dlist, GlobalContext* globalCtx) {

    OPEN_DISP(globalCtx->state.gfxCtx, "../z_debug_display.c", 211);
    func_8009435C(globalCtx->state.gfxCtx);

    gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, dispObj->color.r, dispObj->color.g, dispObj->color.b, dispObj->color.a);

    gSPSetLights1(NEXT_POLY_XLU_DISP, sDebugObjectLights);

    func_800D1694(dispObj->pos.x, dispObj->pos.y, dispObj->pos.z, &dispObj->rot);
    Matrix_Scale(dispObj->scale.x, dispObj->scale.y, dispObj->scale.z, MTXMODE_APPLY);

    gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_debug_display.c", 228),
              G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(NEXT_POLY_XLU_DISP, dlist);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_debug_display.c", 231);
}
