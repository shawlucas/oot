...
void GetItem_DrawMaskOrBombchu(PlayState* play, s16 drawId) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_draw.c", __LINE__);

    Gfx_SetupDL_26Opa(play->state.gfxCtx);
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_draw.c", __LINE__), G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(POLY_OPA_DISP++, sDrawItemTable[drawId].dlists[0]);

    CLOSE_DISPS(play->state.gfxCtx, "../z_draw.c", __LINE__);
}
...
void GetItem_DrawSoldOut(PlayState* play, s16 drawId) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_draw.c", __LINE__), G_MTX_MODELVIEW | G_MTX_LOAD);
