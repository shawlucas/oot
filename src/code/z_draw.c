...
void GetItem_DrawMaskOrBombchu(GlobalContext* globalCtx, s16 drawId) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_draw.c", __LINE__),
              G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(POLY_OPA_DISP++, sDrawItemTable[drawId].dlists[0]);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_draw.c", __LINE__);
}
...
void GetItem_DrawSoldOut(GlobalContext* globalCtx, s16 drawId) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_draw.c", __LINE__),
              G_MTX_MODELVIEW | G_MTX_LOAD);
