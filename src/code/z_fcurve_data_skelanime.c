...
s32 SkelCurve_Init(GlobalContext* globalCtx, SkelAnimeCurve* skelCurve, SkelCurveLimbList* limbListSeg,
    ...
    skelCurve->transforms = ZeldaArena_MallocDebug(sizeof(*skelCurve->transforms) * skelCurve->limbCount,
                                                   "../z_fcurve_data_skelanime.c", __LINE__);
    ASSERT(skelCurve->transforms != NULL, "skelCurve->transforms != NULL", "../z_fcurve_data_skelanime.c", __LINE__);
    skelCurve->animCurFrame = 0.0f;
    return 1;
}
...
void SkelCurve_Destroy(GlobalContext* globalCtx, SkelAnimeCurve* skelCurve) {
    if (skelCurve->transforms != NULL) {
        ZeldaArena_FreeDebug(skelCurve->transforms, "../z_fcurve_data_skelanime.c", __LINE__);
    }
}
...
void SkelCurve_DrawLimb(GlobalContext* globalCtx, s32 limbIndex, SkelAnimeCurve* skelCurve,
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_fcurve_data_skelanime.c", __LINE__);
    ...
    if (overrideLimbDraw == NULL ||
        ...
        if (lod == 0) {
            ...
            dList = limb->dList[0];
            if (dList != NULL) {
                gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_fcurve_data_skelanime.c", __LINE__),
            ...
            dList = limb->dList[1];
            if (dList != NULL) {
                gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_fcurve_data_skelanime.c", __LINE__),
                          G_MTX_LOAD | G_MTX_NOPUSH | G_MTX_MODELVIEW);
                gSPDisplayList(POLY_XLU_DISP++, dList);
            }
        } else {
            // "FcSkeletonInfo_draw_child (): Not supported"
            osSyncPrintf("SkelCurve_DrawLimb():Not supported\n");
        }
    }
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_fcurve_data_skelanime.c", __LINE__);
}
