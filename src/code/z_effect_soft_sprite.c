...
void EffectSs_Spawn(PlayState* play, s32 type, s32 priority, void* initParams) {
    ...
    if (overlayEntry->vramStart == NULL) {
        // "Not an overlay"
        PRINTF("EffectSs_Spawn():Not an overlay\n");
        ...
        if (overlayEntry->loadedRamAddr == NULL) {
            ...
            if (overlayEntry->loadedRamAddr == NULL) {
                ...
                PRINTF("EffectSs_Spawn():ZeldaArena_MallocR()により,%"
                       "cannot reserve %d byte memory.\n this means the program cannot be loaded.\n"
                       "a dangerous situation! effects won't appear either, naturally.\n",
                       overlaySize);
                PRINTF(VT_RST);
                return;
            }
    ...
    if (initInfo->init == NULL) {
        ...
        PRINTF("EffectSs_Spawn():Effects have already been loaded, but the constructor is NULL so the addition will not occur.\n"
        "Please fix this. (Waste of memory) %08x %d\n",
               initInfo, type);
        return;
    }
    ...
    for (i = 0; i < sEffectSsInfo.tableSize; i++) {
        if (sEffectSsInfo.table[i].life > -1) {
            if ((sEffectSsInfo.table[i].pos.x > 32000.0f) || (sEffectSsInfo.table[i].pos.x < -32000.0f) ||
                ...
                PRINTF("EffectSs_DrawAll():位置が領域外のため "
