...
void Gameplay_Destroy(GlobalContext* globalCtx) {
    ...
    globalCtx->state.gfxCtx->TaskEndCallBack = NULL;
    globalCtx->state.gfxCtx->TaskEndClientData = 0;
    SREG(91) = 0;
    R_PAUSE_MENU_MODE = 0;
...
void Gameplay_Init(GlobalContext* globalCtx) {
    ...
    func_80057C6C(&globalCtx->camera, &globalCtx->view, &globalCtx->colCtx, globalCtx);
    Camera_ChangeStatus(&globalCtx->camera, 7);

    for (i = 0; i < 3; i++) {
        func_80057C6C(&globalCtx->subCameras[i + 1], &globalCtx->view, &globalCtx->colCtx, globalCtx);
        Camera_ChangeStatus(&globalCtx->subCameras[i + 1], 0x100);
    }

    globalCtx->cameraPtrs[0] = &globalCtx->camera;
    ...
    if (gSaveContext.nextDayTime != 0xFFFFU) {
        gSaveContext.zeldaTime = gSaveContext.nextDayTime;
        gSaveContext.environmentTime = gSaveContext.nextDayTime;
    }

    if ((gSaveContext.zeldaTime >= 0xC001) || (gSaveContext.zeldaTime < 0x4555)) {
    ...
    if ((gSaveContext.gameInfo.gameMode != 0) || (gSaveContext.cutsceneIndex >= 0xFFF0)) {
        gSaveContext.gameInfo.nayrusLoveTimer = 0;
        func_800876C8(globalCtx);
        gSaveContext.gameInfo.sceneSetupIndex = (gSaveContext.cutsceneIndex & 0xF) + 4;
    } else if (LINK_IS_CHILD && (gSaveContext.nightFlag == 0)) {
        gSaveContext.gameInfo.sceneSetupIndex = 0;
    } else if (LINK_IS_CHILD && (gSaveContext.nightFlag != 0)) {
        gSaveContext.gameInfo.sceneSetupIndex = 1;
    } else if (LINK_IS_ADULT && (gSaveContext.nightFlag == 0)) {
        gSaveContext.gameInfo.sceneSetupIndex = 2;
    } else {
        gSaveContext.gameInfo.sceneSetupIndex = 3;
    }

    tempSetupIndex = gSaveContext.gameInfo.sceneSetupIndex;
    if ((gEntranceTable[gSaveContext.entranceIndex].scene == SCENE_SPOT00) && LINK_IS_CHILD &&
        (gSaveContext.gameInfo.sceneSetupIndex < 4)) {
    ...
    spawnEntrance = &gEntranceTable[gSaveContext.entranceIndex + gSaveContext.gameInfo.sceneSetupIndex];
    Gameplay_SpawnScene(globalCtx, spawnEntrance->scene, spawnEntrance->spawn);
    osSyncPrintf("\nSCENE_NO=%d COUNTER=%d\n", gSaveContext.entranceIndex, gSaveContext.gameInfo.sceneSetupIndex);

    // When entering Gerudo Valley in the right setup, trigger the GC emulator to play the ending movie.
    // The emulator constantly checks whether PC is 0x81000000, so this works even though it's not a valid address.
    if ((gEntranceTable[gSaveContext.entranceIndex].scene == SCENE_SPOT09) && (gSaveContext.gameInfo.sceneSetupIndex == 6)) {
    ...
    if (gSaveContext.nextDayTime != 0xFFFF) {
        if (gSaveContext.nextDayTime == 0x8001) {
            gSaveContext.totalDay++;
            gSaveContext.eventDay++;
    ...
    if (gSaveContext.gameInfo.gameMode != 1) {
    ...
    player = PLAYER;
    func_80058148(&globalCtx->camera, player);
    func_8005A444(&globalCtx->camera, 0);

    playerStartCamId = player->actor.params & 0xFF;
    if (playerStartCamId != 0xFF) {
        osSyncPrintf("player has start camera ID (" VT_FGCOL(BLUE) "%d" VT_RST ")\n", playerStartCamId);
        func_8005A7A8(&globalCtx->camera, playerStartCamId);
    }
...
void Gameplay_Update(GlobalContext* globalCtx) {
    ...
    if (func_8008E6AC(&globalCtx->pause, &input[1]) != 0) {
                ...
                case 3:
                    if (globalCtx->transitionCtx.isDone(&globalCtx->transitionCtx) != 0) {
                        if (globalCtx->transitionCtx.transitionType >= 56) {
                            ...
                            if (gSaveContext.gameInfo.gameMode != 2) {
        ...
        if (1 && (gTrnsnUnkState != 3)) {
            ...
            if ((gSaveContext.gameInfo.gameMode == 0) && (globalCtx->msgCtx.msgMode == 0) && (globalCtx->goverCtx == 0)) {
...
void Gameplay_Draw(GlobalContext* globalCtx) {
    ...
    gfxCtx = globalCtx->state.gfxCtx;
    OPEN_DISP(gfxCtx, "../z_play.c", 3907);
    ...
    gSPSegment(NEXT_POLY_XLU_DISP, 0x00, NULL);
    gSPSegment(NEXT_OVERLAY_DISP, 0x00, NULL);

    gSPSegment(NEXT_DISP, 0x04, globalCtx->objectCtx.status[globalCtx->objectCtx.mainKeepIndex].segment);
    gSPSegment(NEXT_POLY_XLU_DISP, 0x04, globalCtx->objectCtx.status[globalCtx->objectCtx.mainKeepIndex].segment);
    gSPSegment(NEXT_OVERLAY_DISP, 0x04, globalCtx->objectCtx.status[globalCtx->objectCtx.mainKeepIndex].segment);

    gSPSegment(NEXT_DISP, 0x05, globalCtx->objectCtx.status[globalCtx->objectCtx.subKeepIndex].segment);
    gSPSegment(NEXT_POLY_XLU_DISP, 0x05, globalCtx->objectCtx.status[globalCtx->objectCtx.subKeepIndex].segment);
    gSPSegment(NEXT_OVERLAY_DISP, 0x05, globalCtx->objectCtx.status[globalCtx->objectCtx.subKeepIndex].segment);

    gSPSegment(NEXT_DISP, 0x02, globalCtx->sceneSegment);
    gSPSegment(NEXT_POLY_XLU_DISP, 0x02, globalCtx->sceneSegment);
    gSPSegment(NEXT_OVERLAY_DISP, 0x02, globalCtx->sceneSegment);
    ...
    if ((HREG(80) != 10) || (HREG(82) != 0)) {
        ...
        if ((HREG(80) != 10) || (HREG(92) != 0)) {
            ...
            gSPDisplayList(NEXT_OVERLAY_DISP, gfxP);
        ...
        if (gTrnsnUnkState == 3) {
            ...
            func_800C0ED8(&globalCtx->preRenderCtx, 0x140, 0xF0, gfxCtx->FrameBufferP, gZBuffer);
            ...
            if (R_PAUSE_MENU_MODE == 3) {
                ...
                if ((R_PAUSE_MENU_MODE == 1) || (gTrnsnUnkState == 1)) {
                    ...
                    globalCtx->preRenderCtx.unk_10 = gfxCtx->FrameBufferP;
                    globalCtx->preRenderCtx.unk_14 = gZBuffer;
                    func_800C1F20(&globalCtx->preRenderCtx, &sp70);
                    if (R_PAUSE_MENU_MODE == 1) {
                        globalCtx->preRenderCtx.unk_18 = gfxCtx->FrameBufferP;
    ...
    CLOSE_DISP(gfxCtx, "../z_play.c", 4508);
}
#else
...
s16 Gameplay_CreateSubCamera(GlobalContext* globalCtx) {
    ...
    globalCtx->cameraPtrs[i] = &globalCtx->camera + i;
...
void Gameplay_SaveSceneFlags(GlobalContext* globalCtx) {
    SaveSceneFlags* sceneFlags = &gSaveContext.memory.information.sceneFlags[globalCtx->sceneNum];
...
void Gameplay_SetRespawnData(GlobalContext* globalCtx, s32 respawnMode, s16 entranceIndex, s32 roomIndex,
    ...
    RespawnData* respawnData = &gSaveContext.gameInfo.respawn[respawnMode];
...
void Gameplay_TriggerVoidOut(GlobalContext* globalCtx) {
    gSaveContext.gameInfo.respawn[RESPAWN_MODE_DOWN].tempSwchFlags = globalCtx->actorCtx.flags.tempSwch;
    gSaveContext.gameInfo.respawn[RESPAWN_MODE_DOWN].tempCollectFlags = globalCtx->actorCtx.flags.tempCollect;
    gSaveContext.gameInfo.respawnFlag = 1;
    globalCtx->sceneLoadFlag = 0x14;
    globalCtx->nextEntranceIndex = gSaveContext.gameInfo.respawn[RESPAWN_MODE_DOWN].entranceIndex;
