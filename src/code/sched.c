...
void Sched_SwapFrameBuffer(CfbInfo* cfbInfo) {
    ...
    LogUtils_CheckValidPointer("cfbInfo->swapBuffer", cfbInfo->swapBuffer, "../sched.c", __LINE__);
...
void Sched_HandleReset(SchedContext* sc) {
    ...
    if (sc->curRSPTask != NULL) {
        ...
        if (sc->curRSPTask->framebuffer == NULL) {
            LOG_TIME("(((u64)(now - sRSPAudioStartTime)*(1000000LL/15625LL))/((62500000LL*3/4)/15625LL))",
                     OS_CYCLES_TO_USEC(now - sRSPAudioStartTime), "../sched.c", __LINE__);
        } else if (OS_CYCLES_TO_USEC(now - sRSPGFXStartTime) > 1000000 ||
                   OS_CYCLES_TO_USEC(now - sRDPStartTime) > 1000000) {
            func_800FBFD8();
            if (sc->curRSPTask != NULL) {
                LOG_TIME("(((u64)(now - sRSPGFXStartTime)*(1000000LL/15625LL))/((62500000LL*3/4)/15625LL))",
                         OS_CYCLES_TO_USEC(now - sRSPGFXStartTime), "../sched.c", __LINE__);
                osSendMesg(&sc->interruptQ, RSP_DONE_MSG, OS_MESG_NOBLOCK);
            }
            if (sc->curRDPTask != NULL) {
                LOG_TIME("(((u64)(now - sRDPStartTime)*(1000000LL/15625LL))/((62500000LL*3/4)/15625LL))",
                         OS_CYCLES_TO_USEC(now - sRDPStartTime), "../sched.c", __LINE__);
...
void Sched_QueueTask(SchedContext* sc, OSScTask* task) {
    ...
    if (type == M_AUDTASK) {
        if (sLogScheduler) {
            ...
            osSyncPrintf("Audio task entered\n");
        }
        ...
        if (sLogScheduler) {
            osSyncPrintf("Graph Task Entered\n"); // "Entered graph task"
        }
...
OSScTask* func_800C89D4(SchedContext* sc, OSScTask* task) {
    ...
    if (sc->pendingSwapBuf1 != NULL) {
        if (0) {
            ASSERT(sc->pendingSwapBuf1 != NULL, "sc->pendingSwapBuf1 != NULL", "../sched.c", __LINE__);
        }
        return NULL;
    }
    ...
    if (sc->pendingSwapBuf2 != NULL) {
        if (0) {
            ASSERT(sc->pendingSwapBuf2 != NULL, "sc->pendingSwapBuf2 != NULL", "../sched.c", __LINE__);
        }
        return NULL;
    }
...
void func_800C8BC4(SchedContext* sc, OSScTask* task) {
    if (sc->pendingSwapBuf1 == NULL) {
        ...
        LogUtils_CheckValidPointer("sc->pendingSwapBuf1", sc->pendingSwapBuf1, "../sched.c", __LINE__);
...
void Sched_RunTask(SchedContext* sc, OSScTask* spTask, OSScTask* dpTask) {
    ASSERT(sc->curRSPTask == NULL, "sc->curRSPTask == NULL", "../sched.c", __LINE__);
    if (spTask != NULL) {
...
void Sched_HandleEntry(SchedContext* sc) {
    ...
    if (sLogScheduler) {
        osSyncPrintf("EN sc:%08X nextRSP:%08X nextRDP:%08X state:%x\n", sc, nextRSP, nextRDP, state);
    }
}
...
void Sched_HandleRDPDone(SchedContext* sc) {
    ...
    if (sLogScheduler) {
        osSyncPrintf("DP sc:%08X nextRSP:%08X nextRDP:%08X state:%x\n", sc, nextRSP, nextRDP, state);
    }
}

void Sched_SendEntryMsg(SchedContext* sc) {
    if (sLogScheduler) {
        osSyncPrintf("Sched_SendEntryMsg\n");
    }
