...
void func_80095AB4(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_room.c", 193);

    if (flags & 1) {
        func_800342EC(&D_801270A0, globalCtx);
        gSPSegment(NEXT_DISP, 0x03, room->segment);
        func_80093C80(globalCtx);
        gSPMatrix(NEXT_DISP, &gMtxClear, G_MTX_MODELVIEW | G_MTX_LOAD);
    }

    if (flags & 2) {
        func_8003435C(&D_801270A0, globalCtx);
        gSPSegment(NEXT_POLY_XLU_DISP, 0x03, room->segment);
        func_80093D84(globalCtx->state.gfxCtx);
        gSPMatrix(NEXT_POLY_XLU_DISP, &gMtxClear, G_MTX_MODELVIEW | G_MTX_LOAD);
    }
    ...
    for (i = 0; i < polygon0->num; i++) {
        if ((flags & 1) && (polygonDlist->opa != NULL)) {
            gSPDisplayList(NEXT_DISP, polygonDlist->opa);
        }

        if ((flags & 2) && (polygonDlist->xlu != NULL)) {
            gSPDisplayList(NEXT_POLY_XLU_DISP, polygonDlist->xlu);
        }

        polygonDlist++;
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_room.c", 239);
}
...
void func_80095D04(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    spB0 = NULL;
    spB4 = NULL;
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_room.c", 287);
        ...
        if (iREG(86) != 0) {
            ...
            if (((iREG(86) == 1) && (iREG(89) > sp9C)) || ((iREG(86) == 2) && (iREG(89) == sp9C))) {
                if ((flags & 1) && (phi_s0->opa != NULL)) {
                    gSPDisplayList(NEXT_DISP, phi_s0->opa);
                }

                if ((flags & 2) && (phi_s0->xlu != NULL)) {
                    gSPDisplayList(NEXT_POLY_XLU_DISP, phi_s0->xlu);
                }
            }
        } else {
            if ((flags & 1) && (phi_s0->opa != NULL)) {
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_room.c", 430);
}
#else
...
void func_80096680(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_room.c", 628);
    ...
    if (sp94 || sp98) {
        ...
        if (sp98) {
            // gSPLoadUcodeL(NEXT_DISP, rspS2DEX)?

            gSPLoadUcodeEx(NEXT_DISP, OS_K0_TO_PHYSICAL(D_80113070), OS_K0_TO_PHYSICAL(D_801579A0), 0x800);
            {
                Vec3f sp60;
                spA8 = NOW_DISP;
            ...
            // gSPLoadUcode(NEXT_DISP, SysUcode_GetUCode(), SysUcode_GetUCodeData())?
            gSPLoadUcodeEx(NEXT_DISP, SysUcode_GetUCode(), SysUcode_GetUCodeData(), 0x800);
        }
    }
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_room.c", 691);
}
...
void func_80096B6C(GlobalContext* globalCtx, Room* room, u32 flags) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_room.c", 752);
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_room.c", 819);
}
#else
