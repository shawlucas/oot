...
void Math3D_LineClosestToPoint(InfiniteLine* line, Vec3f* pos, Vec3f* closestPoint) {
    ...
    if (IS_ZERO(dirVectorLengthSq)) {
        ...
        PRINTF("Math3D_LineClosestToPoint(): No straight line length\n");
        PRINTF("Returns cross = pos\n"); // "Returns cross = pos."
        PRINTF(VT_RST);
...
f32 Math3D_UDistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    if (OOT_DEBUG && IS_ZERO(sqrtf(SQ(nx) + SQ(ny) + SQ(nz)))) {
        ...
        PRINTF("Math3D_UDistPlaneToPos(): Normal size is near zero %f %f %f\n", nx, ny, nz);
        PRINTF(VT_RST);
        return 0.0f;
    }
...
f32 Math3D_DistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    ...
    if (IS_ZERO(normMagnitude)) {
        ...
        PRINTF("Math3D_DistPlaneToPos(): Normal size is close to zero %f %f %f\n", nx, ny, nz);
        PRINTF(VT_RST);
        return 0.0f;
    }
