...
void Math3D_LineClosestToPoint(Linef* line, Vec3f* pos, Vec3f* closestPoint) {
    ...
    if (IS_ZERO(dirVectorSize)) {
        ...
        osSyncPrintf("Math3D_LineClosestToPoint(): No straight line length\n");
...
f32 Math3D_UDistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    ...
    if (IS_ZERO(sqrtf(SQ(nx) + SQ(ny) + SQ(nz)))) {
        ...
        osSyncPrintf("Math3D_UDistPlaneToPos(): Normal size is near zero %f %f %f\n", nx, ny, nz);
        osSyncPrintf(VT_RST);
        return 0.0f;
    }
...
f32 Math3D_DistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    ...
    if (IS_ZERO(normMagnitude)) {
        ...
        osSyncPrintf("Math3D_DistPlaneToPos(): Normal size is close to zero %f %f %f\n", nx, ny, nz);
        osSyncPrintf(VT_RST);
        return 0.0f;
    }
