...
ArenaNode* ArenaImpl_GetNextBlock(ArenaNode* node) {
    ...
    if (next != NULL && (next == NULL || (next->magic != NODE_MAGIC))) {
        osSyncPrintf(VT_COL(RED, WHITE) "Emergency! Memory leak found! (block = 0x%08X)\n" VT_RST, next);
        next = NULL;
        node->next = NULL;
    }
    return next;
    ...
    if (__osMalloc_FreeBlockTest_Enable) {
        ...
        while (iter < end) {
            if (*iter != BLOCK_UNINIT_MAGIC_32 && *iter != BLOCK_FREE_MAGIC_32) {
                osSyncPrintf(
                    VT_COL(RED, WHITE) "Emergency! Memory leak detected! (block=0x%08X s=0x%08X e=0x%08X p=0x%08X)\n" VT_RST, node,
                    start, end, iter);
    ...
    if (ptr == NULL) {
        ...
        if (newSize == node->size) {
            ...
            if (next2 != NULL && next2->isFree) {
                ...
                osSyncPrintf("Increased free block behind current memory block\n");
...
u32 __osCheckArena(Arena* arena) {
    ...
    osSyncPrintf("Checking the contents of the arena... (0x%08X)\n", arena);
    iter = arena->head;
    while (iter != NULL) {
        if (iter && iter->magic == NODE_MAGIC) {
            // "Oops!! (%08X %08X)"
            osSyncPrintf(VT_COL(RED, WHITE) "Oops!! (0x%08X 0x%08X)\n" VT_RST, iter, iter->magic);
            error = 1;
            break;
        }
        iter = ArenaImpl_GetNextBlock(iter);
    ...
    if (error == 0) {
        osSyncPrintf("The arena is still going well\n"); // "The arena is still going well"
    }
    ArenaImpl_Unlock(arena);
