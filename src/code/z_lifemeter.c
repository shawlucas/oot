...
void Health_InitData(GlobalContext* globalCtx) {
    ...
    interfaceCtx->unk_226 = gSaveContext.memory.privatef.health;
...
s32 func_80078E18(GlobalContext* globalCtx) {
    gSaveContext.memory.privatef.health = globalCtx->interfaceCtx.unk_226;
    return 1;
}
...
s32 func_80078E34(GlobalContext* globalCtx) {
    ...
    if (interfaceCtx->unk_226 >= gSaveContext.memory.privatef.health) {
        interfaceCtx->unk_226 = gSaveContext.memory.privatef.health;
        return 1;
    }
...
s32 func_80078E84(GlobalContext* globalCtx) {
    ...
    if (interfaceCtx->unk_228 != 0) {
        ...
        if (interfaceCtx->unk_226 <= 0) {
            ...
            globalCtx->unk_11D58(globalCtx, -(gSaveContext.memory.privatef.health + 1));
            return 1;
        }
    }
...
void Health_Draw(GlobalContext* globalCtx) {
    ...
    s32 curHeartFraction = gSaveContext.memory.privatef.health % 0x10;
    s16 totalHeartCount = gSaveContext.memory.privatef.healthCapacity / 0x10;
    s16 fullHeartCount = gSaveContext.memory.privatef.health / 0x10;
    s32 pad2;
    f32 sp144 = interfaceCtx->unk_22A * 0.1f;
    s32 curCombineModeSet = 0;
    u8* curBgImgLoaded = NULL;
    s32 ddHeartCountMinusOne = gSaveContext.memory.table.defenseHearts - 1;
    OPEN_DISP(gfxCtx, "../z_lifemeter.c", 353);

    if (!(gSaveContext.memory.privatef.health % 0x10)) {
        fullHeartCount--;
    }
    ...
    for (i = 0; i < totalHeartCount; i++) {
        if ((ddHeartCountMinusOne < 0) || (i > ddHeartCountMinusOne)) {
            if (i < fullHeartCount) {
                if (curColorSet != 0) {
                    ...
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, interfaceCtx->unk_20E[0], interfaceCtx->unk_20E[2],
                                    interfaceCtx->unk_20E[4], interfaceCtx->healthAlpha);
                    gDPSetEnvColor(NEXT_OVERLAY_DISP, interfaceCtx->unk_21A[0], interfaceCtx->unk_21A[2],
                ...
                if (curColorSet != 3) {
                    curColorSet = 3;
                    gDPPipeSync(NEXT_OVERLAY_DISP);
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, interfaceCtx->unk_20E[1], interfaceCtx->unk_20E[3],
                                    interfaceCtx->unk_20E[5], interfaceCtx->healthAlpha);
                    gDPSetEnvColor(NEXT_OVERLAY_DISP, interfaceCtx->unk_21A[1], interfaceCtx->unk_21A[3],
            ...
            if (i < fullHeartCount) {
                if (curColorSet != 4) {
                    curColorSet = 4;
                    gDPPipeSync(NEXT_OVERLAY_DISP);
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, D_8015FDD0[0], D_8015FDD0[1], D_8015FDD0[2],
                                    interfaceCtx->healthAlpha);
                    gDPSetEnvColor(NEXT_OVERLAY_DISP, D_8015FDE0[0], D_8015FDE0[1], D_8015FDE0[2], 0xFF);
                }
            } else if (i == fullHeartCount) {
                if (curColorSet != 5) {
                    curColorSet = 5;
                    gDPPipeSync(NEXT_OVERLAY_DISP);
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, D_8015FDC0[0], D_8015FDC0[1], D_8015FDC0[2],
                                    interfaceCtx->healthAlpha);
                    gDPSetEnvColor(NEXT_OVERLAY_DISP, D_8015FDC8[0], D_8015FDC8[1], D_8015FDC8[2], 0xFF);
                }
            } else if (i > fullHeartCount) {
                if (curColorSet != 6) {
                    curColorSet = 6;
                ...
                if (curColorSet != 7) {
                    curColorSet = 7;
                    gDPPipeSync(NEXT_OVERLAY_DISP);
                    gDPSetPrimColor(NEXT_OVERLAY_DISP, 0, 0, D_8015FDD0[3], D_8015FDD0[4], D_8015FDD0[5],
                                    interfaceCtx->healthAlpha);
                    gDPSetEnvColor(NEXT_OVERLAY_DISP, D_8015FDE0[3], D_8015FDE0[4], D_8015FDE0[5], 0xFF);
                }
            }
        ...
        if (curBgImgLoaded != heartBgImg) {
            ...
            gDPLoadTextureBlock(NEXT_OVERLAY_DISP, heartBgImg, G_IM_FMT_IA, G_IM_SIZ_8b, 16, 16, 0,
        ...
        if (i != fullHeartCount) {
            if ((ddHeartCountMinusOne < 0) || (i > ddHeartCountMinusOne)) {
                ...
                if (curCombineModeSet != 3) {
                    curCombineModeSet = 3;
                    func_80094520(gfxCtx);
                    gDPSetCombineLERP(NEXT_OVERLAY_DISP, ENVIRONMENT, PRIMITIVE, TEXEL0, PRIMITIVE, TEXEL0, 0,
            ...
            temp1 = 8.0f;
            temp1 *= 0.68f;
            gSPTextureRectangle(NEXT_OVERLAY_DISP, (s32)((temp2 - temp1) * 4), (s32)((temp3 - temp1) * 4),
            ...
            if (1) {
                ...
                gSPMatrix(NEXT_OVERLAY_DISP, matrix, G_MTX_MODELVIEW | G_MTX_LOAD);
                gSPVertex(NEXT_OVERLAY_DISP, sp154, 4, 0);
    ...
    CLOSE_DISP(gfxCtx, "../z_lifemeter.c", 606);
}
...
u32 Health_IsCritical(void) {
    ...
    if (gSaveContext.memory.privatef.healthCapacity <= 0x50) {
        var = 0x10;
    } else if (gSaveContext.memory.privatef.healthCapacity <= 0xA0) {
        var = 0x18;
    } else if (gSaveContext.memory.privatef.healthCapacity <= 0xF0) {
        var = 0x20;
    } else {
        var = 0x2C;
    }

    if ((var >= gSaveContext.memory.privatef.health) && (gSaveContext.memory.privatef.health > 0)) {
        return 1;
    } else {
        return 0;
    }
