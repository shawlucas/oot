...
void Interface_Init(PlayState* play) {
    ...
    osSyncPrintf("Permanent ＰＡＲＡＭＥＴＥＲ Segment=%x\n", parameterSize);

    interfaceCtx->parameterSegment = GameState_Alloc(&play->state, parameterSize, "../z_construct.c", __LINE__);
    ...
    interfaceCtx->doActionSegment = GameState_Alloc(&play->state, 3 * DO_ACTION_TEX_SIZE, "../z_construct.c", __LINE__);

    osSyncPrintf("ＤＯ Action Texture Initialization=%x\n", 3 * DO_ACTION_TEX_SIZE); // "DO Action Texture Initialization"
    ...
    DmaMgr_RequestSyncDebug(interfaceCtx->doActionSegment, (uintptr_t)_do_action_staticSegmentRomStart + doActionOffset,
                            2 * DO_ACTION_TEX_SIZE, "../z_construct.c", __LINE__);
...
void Message_Init(PlayState* play) {
    ...
    msgCtx->textboxSegment = GameState_Alloc(&play->state, TEXTBOX_SEGMENT_SIZE, "../z_construct.c", __LINE__);

    osSyncPrintf("msgCtx->textboxSegment=%x\n", msgCtx->textboxSegment);

    osSyncPrintf("TEXTBOX_SEGMENT_SIZE=%x\n", TEXTBOX_SEGMENT_SIZE); // "Textbox game_alloc=%x"
    ASSERT(msgCtx->textboxSegment != NULL, "msgCtx->textboxSegment != NULL", "../z_construct.c", __LINE__);
