...
void Sram_VerifyAndLoadAllSaves(FileSelectState* fileSelect, SramContext* sramCtx) {
    ...
    for (slotNum = 0; slotNum < 3; slotNum++) {
        ...
        osSyncPrintf("\nＳＡＶＥ checksum calculation  newChecksum=%x  oldChecksum=%x  ", newChecksum, oldChecksum);

        if (newChecksum != oldChecksum) {
            // checksum didnt match, try backup save
            osSyncPrintf("CHECKSUM ERROR! TRY BACKUP SAVE! %x(%d)\n", gSramSlotOffsets[slotNum], slotNum);
            ...
            osSyncPrintf("\n（Ｂ）ＳＡＶＥ CHECKSUM CALCULATION  j=%x  mmm=%x  ", newChecksum, oldChecksum);

            if (newChecksum != oldChecksum) {
                // backup save didnt work, make new save
                osSyncPrintf("ＥＲＲＯＲ BACKUP SAVE DIDNT WORK, MAKE NEW SAVE %x(%d+3)\n", gSramSlotOffsets[slotNum + 3], slotNum);
            ...
            osSyncPrintf("Point=%x(%d)  checkSum=%x(%x)\n", i, slotNum, gSaveContext.save.info.checksum,
                         newChecksum);
        } else {
            osSyncPrintf("\nＳＡＶＥ DATA ＯＫ！！！！\n"); // "SAVE data OK! ! ! !"
        }
...
void Sram_EraseSave(FileSelectState* fileSelect, SramContext* sramCtx) {
    ...
    osSyncPrintf("Sram_EraseSave END\n");
}
...
void Sram_InitSram(GameState* gameState, SramContext* sramCtx) {
    ...
    osSyncPrintf("ＧＯＯＤ！ＧＯＯＤ！ Size = %d + %d ＝ %d\n", sizeof(SaveInfo), 4, sizeof(SaveInfo) + 4);
    osSyncPrintf(VT_FGCOL(BLUE));
    osSyncPrintf("gSaveContext.audioSetting = %d\n", gSaveContext.audioSetting);
    osSyncPrintf("gSaveContext.audioSetting = %d\n", gSaveContext.audioSetting);
    osSyncPrintf("gSaveContext.audioSetting = %d\n", gSaveContext.audioSetting);
    osSyncPrintf(VT_RST);
...
void Sram_Alloc(GameState* gameState, SramContext* sramCtx) {
    sramCtx->readBuff = GameState_Alloc(gameState, SRAM_SIZE, "../z_sram.c", __LINE__);
