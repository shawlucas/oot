...
void EffectSpark_Init(void* thisx, void* initParamsx) {
    ...
    if ((this != NULL) && (initParams != NULL)) {
        if ((initParams->uDiv == 0) || (initParams->vDiv == 0)) {
            PRINTF("EffectSpark_Init():u_div,v_div 0 is not good\n"); // "u_div,v_div 0 is not good."
            return;
        }
...
s32 EffectSpark_Update(void* thisx) {
    ...
    if (this == NULL) {
        PRINTF("EffectSpark_Update():Spark Pointer is NULL\n");
    }
...
void EffectSpark_Draw(void* thisx, GraphicsContext* gfxCtx) {
    ...
    if (this != NULL) {
        ...
        if (vertices == NULL) {
            ...
            PRINTF("EffectSpark_Draw(): Graph_Alloc Failure\n");
            goto end;
        }
