...
void EffectSpark_Init(void* thisx, void* initParamsx) {
    ...
    if ((this != NULL) && (initParams != NULL)) {
        if ((initParams->uDiv == 0) || (initParams->vDiv == 0)) {
            osSyncPrintf("EffectSpark_Init():u_div,v_div 0 is not good.。\n"); // "u_div,v_div 0 is not good."
            return;
        }
        ...
        if (this->numElements > ARRAY_COUNT(this->elements)) {
            osSyncPrintf("overlay tablesize\n"); // "over table_size"
            return;
        }
...
void EffectSpark_Draw(void* thisx, GraphicsContext* gfxCtx) {
    ...
    OPEN_DISPS(gfxCtx, "../z_eff_spark.c", __LINE__);
    ...
    if (this != NULL) {
        ...
        if (vertices == NULL) {
            ...
            osSyncPrintf("EffectSpark_Draw():Memory Allocation Failure Graph_Alloc\n");
            goto end;
        }
    ...
    CLOSE_DISPS(gfxCtx, "../z_eff_spark.c", __LINE__);
}
