...
int vector_reserve(struct vector *vector, size_t num)
{
  ...
  if (new_cap <= vector->capacity)
    return 1;
  new_data = (char*)DebugArena_ReallocDebug(vector->begin, vector->element_size * new_cap, __FILE__, __LINE__);
  if (!new_data)
    return 0;
                    ...
                    const void *data)
{
  ...
  if (new_cap != vector->capacity) {
    char *new_data = (char*)DebugArena_ReallocDebug(vector->begin,
                                    vector->element_size * new_cap, __FILE__, __LINE__);
    if (!new_data)
      return 0;
    vector->begin = new_data;
    vector->rend = new_data - vector->element_size;
...
{
  if (vector->begin)
    DebugArena_FreeDebug(vector->begin, __FILE__, __LINE__);
}
