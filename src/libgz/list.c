...
void *list_prev(const void *element)
{
  struct list_element_header *header =
      (struct list_element_header*)((const char*)element -
                                          ELEMENT_HEADER_SIZE);
  if (header->prev)
                       ...
                       const void *data)
{
  ...
  struct list_element_header *new_header = (struct list_element_header*)
                                           DebugArena_MallocDebug(ELEMENT_HEADER_SIZE + size, __FILE__, __LINE__);
  if (!new_header)
    return NULL;
...
void list_erase(struct list *list, void *element)
{
  ...
  DebugArena_FreeDebug(header, __FILE__, __LINE__);
  --list->size;
}
