...
void StackCheck_Init(StackEntry* entry, void* stackTop, void* stackBottom, u32 initValue, s32 minSpace,
    ...
    if (entry == NULL) {
        ...
        while (iter) {
            if (iter == entry) {
                osSyncPrintf(VT_COL(RED, WHITE) "StackCheck_Init: 0x%08X is already in the list\n" VT_RST, entry);
                return;
            }
            iter = iter->next;
        }
...
void StackCheck_Cleanup(StackEntry* entry) {
    ...
    if (inconsistency) {
        osSyncPrintf(VT_COL(RED, WHITE) "StackCheck_Cleanup: 0x%08X list inconsistency\n" VT_RST, entry);
    }
}
...
StackStatus StackCheck_GetState(StackEntry* entry) {
    ...
    osSyncPrintf("head=0x%08X tail=0x%08X last=0x%08X used=0x%08X free=0x%08X [%s]\n", entry->head, entry->tail, last,
                 used, free, entry->name != NULL ? entry->name : "(null)");
    osSyncPrintf(VT_RST);
