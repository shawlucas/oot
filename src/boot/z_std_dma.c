...
void DmaMgr_ProcessMsg(DmaRequest* req) {
        ...
        if (vrom >= iter->vromStart && vrom < iter->vromEnd) {
            ...
            if (iter->romEnd == 0) {
                if (iter->vromEnd < vrom + size) {
                    ...
                    #else
                    Fault_AddHungupAndCrash("../z_std_dma.c", 726);
                    #endif
                }
                ...
                if (vrom != iter->vromStart) {
                    ...
                    #else
                    Fault_AddHungupAndCrash("../z_std_dma.c", 746);
                    #endif
                }
                ...
                if (size != iter->vromEnd - iter->vromStart) {
                    ...
                    #else
                    Fault_AddHungupAndCrash("../z_std_dma.c", 752);
                    #endif
                }
    ...
    if (!found) {
        if (sDmaMgrDataExistError) {
            ...
            #else
            Fault_AddHungupAndCrash("../z_std_dma.c", 771);
            #endif
            return;
        }
