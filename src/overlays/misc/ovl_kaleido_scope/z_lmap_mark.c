...
void PauseMapMark_DrawForDungeon(GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_lmap_mark.c", __LINE__);
    ...
    while (true) {
        ...
        for (i = 0; i < mapMarkData->count; i++) {
            ...
            if (display) {
                ...
                gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_lmap_mark.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_lmap_mark.c", __LINE__);
}
