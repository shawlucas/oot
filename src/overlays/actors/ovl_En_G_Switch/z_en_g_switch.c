...
void EnGSwitch_DrawPot(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!this->broken) {
        ...
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_g_switch.c", __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_OPA_DISP++, object_tsubo_DL_0017C0);
        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_g_switch.c", __LINE__);
    }
}
...
void EnGSwitch_DrawEffects(EnGSwitch* this, GlobalContext* globalCtx) {
    ...
    CLOSE_DISPS(gfxCtx, "../z_en_g_switch.c", __LINE__);
}
