...
void ObjLightswitch_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->actor.params & 1) == 1) {
        ...
        if (Actor_SpawnAsChild(&globalCtx->actorCtx, &this->actor, globalCtx, ACTOR_OBJ_OSHIHIKI,
            ...
            osSyncPrintf("押引ブロック発生失敗(%s %d)(params 0x%04X)\n", "../z_obj_lightswitch.c", 452,
                         this->actor.params);
            osSyncPrintf(VT_RST);
            removeSelf = true;
        }
    ...
    // "Light switch"
    osSyncPrintf("(光スイッチ)(params 0x%04X)\n", this->actor.params);
}
...
void ObjLightswitch_DrawOpa(ObjLightswitch* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_obj_lightswitch.c", __LINE__);
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_obj_lightswitch.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_obj_lightswitch.c", __LINE__);
}
...
void ObjLightswitch_DrawXlu(ObjLightswitch* this, GlobalContext* globalCtx) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_obj_lightswitch.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
