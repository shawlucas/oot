...
u8 EnMd_ShouldSpawn(EnMd* this, GlobalContext* globalCtx) {
    if (globalCtx->sceneNum == SCENE_SPOT04) {
        if (!(gSaveContext.memory.information.eventChkInf[1] & 0x1000) && !(gSaveContext.memory.information.eventChkInf[4] & 1)) {
            return 1;
        }
    }

    if (globalCtx->sceneNum == SCENE_KOKIRI_HOME4) {
        if (((gSaveContext.memory.information.eventChkInf[1] & 0x1000) != 0) || ((gSaveContext.memory.information.eventChkInf[4] & 1) != 0)) {
            if (LINK_IS_CHILD) {
                return 1;
            }
        }
