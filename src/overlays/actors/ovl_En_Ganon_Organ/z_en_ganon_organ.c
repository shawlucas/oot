...
void EnGanonOrgan_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    dorf = (BossGanon*)thisx->attachedA;
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_ganon_organ.c", 205);
    osSyncPrintf("ORGAN DRAW  1\n");
    func_80093D18(globalCtx->state.gfxCtx);
    if ((thisx->params == 1) && (dorf->organFadeTimer != 0xff)) {
        gSPSegment(NEXT_DISP, 0x08, func_80A280BC(globalCtx->state.gfxCtx, dorf));
        gSPSegment(NEXT_DISP, 0x09, func_80A28148(globalCtx->state.gfxCtx, dorf));
    } else {
        gSPSegment(NEXT_DISP, 0x08, EnGanonOrgan_EmptyDList(globalCtx->state.gfxCtx));
        gSPSegment(NEXT_DISP, 0x09, EnGanonOrgan_EmptyDList(globalCtx->state.gfxCtx));
    }
    Matrix_Translate(0.0f, 0.0f, 0.0f, MTXMODE_NEW);
    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_ganon_organ.c", 221),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

    gSPDisplayList(NEXT_DISP, D_80A2CCA8);
    gSPDisplayList(NEXT_DISP, D_80A2EAB0);
    osSyncPrintf("ORGAN DRAW  2\n");
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_ganon_organ.c", 230);
}
