...
s8 EnDog_CanFollow(EnDog* this, GlobalContext* globalCtx) {
    ...
    if (this->collider.base.maskB & 1) {
        ...
        if (gSaveContext.gameInfo.dogParams != 0) {
            return 0;
        }
        gSaveContext.gameInfo.dogParams = (this->actor.params & 0x7FFF);
        return 1;
    }
...
void EnDog_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    followingDog = ((gSaveContext.gameInfo.dogParams & 0x0F00) >> 8);
...
void EnDog_FollowLink(EnDog* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.xzDistFromLink > 400.0f) {
        ...
        gSaveContext.gameInfo.dogParams = 0;
        speed = 0.0f;
...
void EnDog_Wait(EnDog* this, GlobalContext* globalCtx) {
    ...
    if ((gSaveContext.gameInfo.dogParams != 0) && (this->actor.xzDistFromLink < 200.0f)) {
        this->nextBehavior = DOG_RUN;
        this->actionFunc = EnDog_RunAway;
    }
}
...
void EnDog_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_dog.c", 972);
    func_80093D18(globalCtx->state.gfxCtx);

    gDPPipeSync(NEXT_DISP);
    gDPSetEnvColor(NEXT_DISP, colors[this->actor.params & 0xF].r, colors[this->actor.params & 0xF].g,
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_dog.c", 994);
}
