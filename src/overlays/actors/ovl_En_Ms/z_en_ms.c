    ...
    if (dialogState != 4) {
        ...
        if (func_80106BC8(globalCtx) != 0) {
            switch (globalCtx->msgCtx.choiceIndex) {
                case 0: // yes
                    if (gSaveContext.memory.privatef.rupees < sPrices[BEANS_BOUGHT]) {
...
void EnMs_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.entranceIndex == 0x157 && gSaveContext.gameInfo.sceneSetupIndex == 8) { // ride carpet if in credits
