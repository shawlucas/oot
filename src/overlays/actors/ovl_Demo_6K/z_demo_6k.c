...
void Demo6K_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (objBankIndex < 0) {
        ASSERT(0, "0", "../z_demo_6k.c", __LINE__);
    } else {
...
void func_80967FFC(Actor* thisx, GlobalContext* globalCtx) {
...

    {
        ...
        for (i = 0; i < 6; i++) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_6k.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_demo_6k.c", __LINE__);
}
