...
void BgMjin_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_mjin.c", 250);
    if (thisx->params != 0) {
        ...
        gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(&D_06000000));
        dlist = D_06000330;
    } else {
        dlist = D_06000140;
    }
    func_80093D18(globalCtx->state.gfxCtx);
    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_mjin.c", 285),
              G_MTX_NOPUSH | G_MTX_MODELVIEW | G_MTX_LOAD);
    gSPDisplayList(NEXT_DISP, dlist);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_mjin.c", 288);
}
