...
void ArrowIce_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((arrow != NULL) && (arrow->actor.update != NULL) && (this->timer < 255)) {
        ...
        // clang-format off
        OPEN_DISP(globalCtx->state.gfxCtx, "../z_arrow_ice.c", 610);
        // clang-format on
        ...
        if (this->unk_164 > 0) {
            SET_NOW_POLY_XLU_DISP(func_800937C0(NOW_POLY_XLU_DISP));
            gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, 0, (s32)(10.0f * this->unk_164) & 0xFF,
                            (s32)(50.0f * this->unk_164) & 0xFF, (s32)(150.0f * this->unk_164) & 0xFF);
            gDPSetAlphaDither(NEXT_POLY_XLU_DISP, G_AD_DISABLE);
            gDPSetColorDither(NEXT_POLY_XLU_DISP, G_CD_DISABLE);
            gDPFillRectangle(NEXT_POLY_XLU_DISP, 0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);
        }

        // Draw ice on the arrow
        func_80093D84(globalCtx->state.gfxCtx);
        gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0x80, 0x80, 170, 255, 255, this->alpha);
        gDPSetEnvColor(NEXT_POLY_XLU_DISP, 0, 0, 255, 128);
        ...
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arrow_ice.c", 660),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_POLY_XLU_DISP, sTextureDL);
        gSPDisplayList(NEXT_POLY_XLU_DISP,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 511 - (stateFrames * 5) % 512, 0, 128, 32, 1,
                                        511 - (stateFrames * 10) % 512, 511 - (stateFrames * 10) % 512, 4, 16));
        gSPDisplayList(NEXT_POLY_XLU_DISP, sVertexDL);
        CLOSE_DISP(globalCtx->state.gfxCtx, "../z_arrow_ice.c", 676);
    }
}
