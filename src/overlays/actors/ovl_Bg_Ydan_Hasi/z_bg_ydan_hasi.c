...
void BgYdanHasi_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->dyna.actor.params == HASI_WATER_BLOCK || this->dyna.actor.params == HASI_THREE_BLOCKS) {
        ...
        OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_ydan_hasi.c", __LINE__);
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_ydan_hasi.c", __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_XLU_DISP++, gDTWaterPlaneDL);

        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_ydan_hasi.c", __LINE__);
    }
}
