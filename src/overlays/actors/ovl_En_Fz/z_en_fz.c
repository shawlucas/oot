...
void EnFz_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->isActive) {
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fz.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_fz.c", __LINE__);
...
void EnFz_DrawIceSmoke(EnFz* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(gfxCtx, "../z_en_fz.c", __LINE__);
    ...
    for (i = 0; i < ARRAY_COUNT(this->iceSmoke); i++) {
        if (iceSmoke->type > 0) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_en_fz.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    CLOSE_DISPS(gfxCtx, "../z_en_fz.c", __LINE__);
}
