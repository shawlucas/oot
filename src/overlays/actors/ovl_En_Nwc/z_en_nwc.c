...
void EnNwc_DrawChicks(EnNwc* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_nwc.c", __LINE__);
    ...
    for (i = 0; i < this->count; i++, chick++) {
        if (chick->type != CHICK_NONE) {
            ...
            mtx = Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_nwc.c", __LINE__);
    ...
    for (i = 0; i < this->count; i++, chick++) {
        if ((chick->type != CHICK_NONE) && (chick->floorPoly != NULL)) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_nwc.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, gCuccoChickShadowDL);
        }
    }
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_nwc.c", __LINE__);
}
