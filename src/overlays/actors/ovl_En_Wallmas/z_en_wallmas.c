...
void EnWallmas_DrawXlu(EnWallmas* this, GlobalContext* globalCtx) {
    ...
    // clang-format off
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_wallmas.c", 1386);
    // clang-format on

    func_80094044(globalCtx->state.gfxCtx);
    gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, 0, 0, 0, 255);
...
void EnWallMas_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == 2) {
        OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_wallmas.c", 1478);
        Matrix_Push();
        ...
        gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_wallmas.c", 1489), G_MTX_LOAD);
        gSPDisplayList(NEXT_DISP, D_06008688);

        Matrix_Pull();
        CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_wallmas.c", 1495);
    }
}
