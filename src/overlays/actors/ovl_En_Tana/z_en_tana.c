...
void func_80B17FC4(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_tana.c", 148);
    func_80093D18(globalCtx->state.gfxCtx);
    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_tana.c", 152),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_DISP, sDLists1[thisx->params]);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_tana.c", 157);
}

void func_80B1809C(Actor* thisx, GlobalContext* globalCtx) {
    EnTana* this = THIS;

    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_tana.c", 163);
    func_80093D18(globalCtx->state.gfxCtx);
    gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(sUnkSegments[thisx->params]));
    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_tana.c", 169),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_DISP, sDLists1[thisx->params]);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_tana.c", 174);
}
