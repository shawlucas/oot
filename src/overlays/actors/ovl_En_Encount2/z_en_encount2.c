...
void EnEncount2_ParticleDraw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(gfxCtx, "../z_en_encount2.c", __LINE__);
    ...
    if (objBankIndex >= 0) {
        ...
        for (i = 0; i < ARRAY_COUNT(this->particles); particle++, i++) {
            if (particle->isAlive) {
                ...
                gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_encount2.c", __LINE__),
    ...
    CLOSE_DISPS(gfxCtx, "../z_en_encount2.c", __LINE__);
}
