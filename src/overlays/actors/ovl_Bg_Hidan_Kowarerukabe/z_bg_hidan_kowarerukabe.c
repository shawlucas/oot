...
void BgHidanKowarerukabe_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (((this->dyna.actor.params & 0xFF) < CRACKED_STONE_FLOOR) ||
        ...
        osSyncPrintf("Error: Fire Temple's param data for broken wall cannot be determined(%s %d)(params 0x%04X)\n",
                     "../z_bg_hidan_kowarerukabe.c", __LINE__, this->dyna.actor.params);
    ...
    osSyncPrintf("(Wall floor broken with hidan bomb)(params 0x%04X)\n", this->dyna.actor.params);
}
...
void BgHidanKowarerukabe_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_hidan_kowarerukabe.c", __LINE__);

    func_80093D18(globalCtx->state.gfxCtx);

    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_hidan_kowarerukabe.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, sBreakableWallDLists[this->dyna.actor.params & 0xFF]);

    Collider_UpdateSpheres(0, &this->collider);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_hidan_kowarerukabe.c", __LINE__);
}
