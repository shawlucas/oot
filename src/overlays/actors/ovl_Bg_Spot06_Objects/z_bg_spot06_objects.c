...
void BgSpot06Objects_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    osSyncPrintf("spot06 obj nthisx->params=[%d]", thisx->params);
...
void BgSpot06Objects_DrawLakeHyliaWater(BgSpot06Objects* this, GlobalContext* globalCtx) {
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot06_objects.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_spot06_objects.c", __LINE__);
}
