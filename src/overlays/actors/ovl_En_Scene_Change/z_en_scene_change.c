...
void EnSceneChange_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_scene_change.c", 290);

    displayListHead = displayList;
    gSPSegment(NEXT_DISP, 0x0C, displayListHead);

    func_80093D18(globalCtx->state.gfxCtx);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_scene_change.c", 386);
}
