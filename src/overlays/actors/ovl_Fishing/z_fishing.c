...
void Fishing_DrawEffects(FishingEffect* effect, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 100; i++) {
        if (effect->type == FS_EFF_RIPPLE) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_fishing.c", __LINE__),
    ...
    for (i = 0; i < 100; i++) {
        if (effect->type == FS_EFF_WATER_DUST) {
            ...
            gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_fishing.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_fishing.c", __LINE__);
}
