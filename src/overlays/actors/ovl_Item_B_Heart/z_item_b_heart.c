...
void ItemBHeart_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_item_b_heart.c", 506);
    ...
    if (flag) {
        ...
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_item_b_heart.c", 551),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_06001290);
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_06001470);
    } else {
        func_80093D18(globalCtx->state.gfxCtx);
        gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_item_b_heart.c", 557),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_DISP, D_06001290);
        gSPDisplayList(NEXT_DISP, D_06001470);
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_item_b_heart.c", 561);
}
