...
void func_80977EA8(GlobalContext* globalCtx, Gfx* dlist) {
    ...
    OPEN_DISP(gfxCtx, "../z_demo_geff.c", 181);
    func_80093D18(gfxCtx);

    gSPMatrix(NEXT_DISP, Matrix_NewMtx(gfxCtx, "../z_demo_geff.c", 183),
              G_MTX_PUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_DISP, dlist);
    gSPPopMatrix(NEXT_DISP, G_MTX_MODELVIEW);

    CLOSE_DISP(gfxCtx, "../z_demo_geff.c", 188);
}
...
void func_80977F80(DemoGeff* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(gfxCtx, "../z_demo_geff.c", 204);

    gSPSegment(NEXT_DISP, 0x06, globalCtx->objectCtx.status[objBankIndex].segment);
    SetSegmentK0(6, globalCtx->objectCtx.status[objBankIndex].segment);

    // Necessary to match
    if (!globalCtx) {}

    CLOSE_DISP(gfxCtx, "../z_demo_geff.c", 212);
}
