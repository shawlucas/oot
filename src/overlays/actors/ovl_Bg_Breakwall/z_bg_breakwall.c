...
void BgBreakwall_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->bombableWallDList != NULL) {
        OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_breakwall.c", __LINE__);

        func_80093D18(globalCtx->state.gfxCtx);
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_breakwall.c", __LINE__),
        ...
        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_breakwall.c", __LINE__);
    }
}
