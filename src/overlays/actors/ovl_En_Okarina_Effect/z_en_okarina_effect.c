...
void EnOkarinaEffect_ManageStorm(EnOkarinaEffect* this, GlobalContext* globalCtx) {
    ...
    if (((globalCtx->pauseCtx.state == 0) && (globalCtx->goverCtx == 0) && (!globalCtx->msgCtx.msgData) &&
         (func_800C0D28(globalCtx) == 0) && ((globalCtx->transitionMode == 0) || (gSaveContext.gameInfo.gameMode != 0))) ||
        (this->timer >= 250)) {
