...
void BgMoriKaitenkabe_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    osSyncPrintf("Forest Temple object [Rotating Wall (params: 0x%04x)] appears \n", this->dyna.actor.params);
    ...
    if (this->moriTexObjIndex < 0) {
        ...
        osSyncPrintf("[Rotating wall] Bank danger!(%s %d)\n", "../z_bg_mori_kaitenkabe.c", __LINE__);
    } else {
...
void BgMoriKaitenkabe_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_mori_kaitenkabe.c", __LINE__);
    func_80093D18(globalCtx->state.gfxCtx);

    gSPSegment(POLY_OPA_DISP++, 0x08, globalCtx->objectCtx.status[this->moriTexObjIndex].segment);

    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_mori_kaitenkabe.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

    gSPDisplayList(POLY_OPA_DISP++, gMoriKaitenkabeDL);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_mori_kaitenkabe.c", __LINE__);
}
