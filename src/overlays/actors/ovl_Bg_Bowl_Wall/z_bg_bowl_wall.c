...
void BgBowlWall_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_bowl_wall.c", __LINE__);

    func_80093D84(globalCtx->state.gfxCtx);
    gSPSegment(POLY_OPA_DISP++, 0x8,
               Gfx_TexScroll(globalCtx->state.gfxCtx, 0, -2 * (frames = globalCtx->state.frames), 16, 16));
    gDPPipeSync(POLY_OPA_DISP++);
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_bowl_wall.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_bowl_wall.c", __LINE__);
}
