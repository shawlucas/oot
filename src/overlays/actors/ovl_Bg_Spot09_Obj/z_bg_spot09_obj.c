...
void BgSpot09Obj_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == 3) {
        OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_spot09_obj.c", 388);
        func_80093D84(globalCtx->state.gfxCtx);

        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot09_obj.c", 391),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_06008010);

        CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_spot09_obj.c", 396);
    }
}
