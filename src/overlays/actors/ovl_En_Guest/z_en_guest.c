...
static InitChainEntry sInitChain[] = {
    ICHAIN_U8(naviRange, 6, ICHAIN_CONTINUE),
...
s32 EnGuest_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_guest.c", 352);
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_guest.c", 388);
...
void EnGuest_Draw(Actor* thisx, GameState* state) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_guest.c", 404);
    func_80093D18(globalCtx->state.gfxCtx);

    gSPSegment(NEXT_DISP, 0x08, func_80A50708(globalCtx->state.gfxCtx, 0xFF, 0xFF, 0xFF, 0xFF));
    gSPSegment(NEXT_DISP, 0x09, func_80A50708(globalCtx->state.gfxCtx, 0xA0, 0x3C, 0xDC, 0xFF));
    gSPSegment(NEXT_DISP, 0x0A, SEGMENTED_TO_VIRTUAL(D_80A50BA4[this->unk_30E]));

    SkelAnime_DrawSV(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl, this->skelAnime.dListCount,
                     EnGuest_OverrideLimbDraw, NULL, &this->actor);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_guest.c", 421);
}
