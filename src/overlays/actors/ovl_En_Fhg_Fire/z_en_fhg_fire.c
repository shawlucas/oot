...
void EnFhgFire_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1723);

    if (thisx->params == 0x24) {
        func_80093D84(globalCtx->state.gfxCtx);
        gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, 255, 255, 255, (s8)this->unk_160);
        gDPSetEnvColor(NEXT_POLY_XLU_DISP, 165, 255, 75, 0);
        gDPPipeSync(NEXT_POLY_XLU_DISP);
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1745),
        ...
        if (this->fireMode > 0) {
            gDPSetEnvColor(NEXT_POLY_XLU_DISP, 0, 255, 255, 0);
        } else {
            gDPSetEnvColor(NEXT_POLY_XLU_DISP, 165, 255, 75, 0);
        }
        gDPPipeSync(NEXT_POLY_XLU_DISP);
        Matrix_RotateZ((thisx->shape.rot.z / 32768.0f) * 3.1416f, 1);
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1801),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_06012160);
    } else if ((thisx->params == 0x27) || (thisx->params == 0x28) || (thisx->params == 0x29)) {
        func_80093D84(globalCtx->state.gfxCtx);
        gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, 0, 0, 0, (u8)this->unk_188);
        gDPSetEnvColor(NEXT_POLY_XLU_DISP, 90, 50, 95, (s8)(this->unk_188 * 0.5f));
        gDPPipeSync(NEXT_POLY_XLU_DISP);
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1833),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        ...
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_0600FAA0);
    } else {
        osSyncPrintf("FF DRAW 1\n");
        Matrix_Translate(0.0f, -100.0f, 0.0f, 1);
        func_80093D84(globalCtx->state.gfxCtx);
        gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0, 255, 255, 255, (s8)this->unk_160);
        gDPSetEnvColor(NEXT_POLY_XLU_DISP, 0, 255, 30, 0);
        gDPPipeSync(NEXT_POLY_XLU_DISP);
        gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1892),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(NEXT_POLY_XLU_DISP, D_060105E0);
        osSyncPrintf("FF DRAW 2\n");
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_fhg_fire.c", 1900);
}
