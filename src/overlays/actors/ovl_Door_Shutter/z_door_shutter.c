...
Gfx* func_80997838(GlobalContext* globalCtx, DoorShutter* this, Gfx* p) {
    ...
    for (i = 0; i < ARRAY_COUNT(sJabuDoorDLists); i++) {
        ...
        gSPMatrix(p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_door_shutter.c", __LINE__),
    ...
    if (this->dyna.actor.objBankIndex == this->requiredObjBankIndex &&
        ...
        if (this->unk_16C == 3) {
            ...
            if (this->unk_170 != 0.0f) {
                ...
                gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_door_shutter.c", __LINE__),
                          G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        ...
        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_door_shutter.c", __LINE__);
    }
}
