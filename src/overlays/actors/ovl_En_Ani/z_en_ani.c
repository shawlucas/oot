...
void func_809B0558(EnAni* this, GlobalContext* globalCtx) {
    if (func_8002F410(&this->actor, globalCtx) != 0) {
        ...
        gSaveContext.memory.information.itemGetInf[1] |= 0x20;
        return;
    }
...
void func_809B064C(EnAni* this, GlobalContext* globalCtx) {
    ...
    if (func_8002F194(&this->actor, globalCtx) != 0) {
        ...
        if (gSaveContext.memory.information.itemGetInf[1] & 0x20) {
    ...
    if (func_8002F194(&this->actor, globalCtx) != 0) {
        if (this->actor.textId == 0x5056) { // "To get a good view..."
            ...
            EnAni_SetupAction(this, func_809B0524);
               ...
               -80.0f < this->actor.yDistFromLink) {
        if ((gSaveContext.memory.information.itemGetInf[1] & 0x20) != 0) {
            EnAni_SetText(this, globalCtx, 0x5056); // "To get a good view..."
        } else {
            EnAni_SetText(this, globalCtx, 0x5055); // "...I'll give you this as a memento."
        }
    } else if (yawDiff >= -0x3E7 && yawDiff < 0x36B0 && this->actor.xzDistFromLink < 350.0f) {
        if ((gSaveContext.memory.information.eventChkInf[2] & 0x8000) == 0) {
            textId = 0x5052; // "...Something is happening on Death Mountain!"
        } else {
            textId = (gSaveContext.memory.information.itemGetInf[1] & 0x20) ? 0x5054 : 0x5053; // "I don't feel like getting down..."
...
void EnAni_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_ani.c", 719);
    func_800943C8(globalCtx->state.gfxCtx);

    gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(D_809B0F80[this->unk_2AC]));

    SkelAnime_DrawSV(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl, this->skelAnime.dListCount,
                     EnAni_OverrideLimbDraw, EnAni_PostLimbDraw, &this->actor);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_ani.c", 736);
}
