...
void EnBa_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_ba.c", __LINE__);
    ...
    if (this->actor.params < EN_BA_DEAD_BLOB) {
        ...
        Matrix_Pop();
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_ba.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_ba.c", __LINE__);
}
