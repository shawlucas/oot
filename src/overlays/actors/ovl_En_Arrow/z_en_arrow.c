...
void EnArrow_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params <= ARROW_0E) {
        ...
        OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_arrow.c", __LINE__);
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_arrow.c", __LINE__),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_XLU_DISP++, gEffSparklesDL);
        Matrix_Pop();
        Matrix_RotateY(this->actor.world.rot.y * (M_PI / 0x8000), MTXMODE_APPLY);

        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_arrow.c", __LINE__);
    }
