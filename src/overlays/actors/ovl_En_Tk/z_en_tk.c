...
void EnTkEff_Draw(EnTk* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_tk_eff.c", __LINE__);
    ...
    for (i = 0; i < ARRAY_COUNT(this->eff); i++) {
        if (eff->active != 0) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_tk_eff.c", __LINE__),
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_tk_eff.c", __LINE__);
}
...
void func_80B1D200(GlobalContext* globalCtx) {
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_tk.c", __LINE__);

    gSPDisplayList(POLY_OPA_DISP++, gDampeShovelDL);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_tk.c", __LINE__);
}
