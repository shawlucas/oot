...
void EnPoDesert_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx,
    ...
    if (limbIndex == 7) {
        ...
        if (CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_7)) {
            ...
            gSPMatrix((*gfxP)++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_po_desert.c", __LINE__),
...
void EnPoDesert_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_po_desert.c", __LINE__);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_po_desert.c", __LINE__);
}
