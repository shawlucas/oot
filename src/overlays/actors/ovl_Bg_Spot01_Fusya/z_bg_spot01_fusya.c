...
void BgSpot01Fusya_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.gameInfo.sceneSetupIndex < 4) {
        gSaveContext.memory.information.eventChkInf[6] &= 0xFFDF;
    }
...
void func_808AAA50(BgSpot01Fusya* this, GlobalContext* globalCtx) {
    ...
    if (gSaveContext.memory.information.eventChkInf[6] & 0x20) {
...
void BgSpot01Fusya_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_spot01_fusya.c", 210);
    func_80093D18(globalCtx->state.gfxCtx);

    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot01_fusya.c", 214),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_DISP, D_06000100);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_spot01_fusya.c", 219);
}
