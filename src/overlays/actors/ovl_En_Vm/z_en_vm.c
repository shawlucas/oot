...
void EnVm_Draw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_vm.c", __LINE__);
    ...
    if (this->unk_260 >= 3) {
        ...
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_vm.c", __LINE__),
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_vm.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, gBeamosLaserDL);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_vm.c", __LINE__);
}
