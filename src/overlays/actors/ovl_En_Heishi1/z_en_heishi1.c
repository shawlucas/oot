...
void EnHeishi1_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    // "current time"
    // clang-format off
    time = gSaveContext.zeldaTime; osSyncPrintf(VT_FGCOL(GREEN) " 今時間 %d\n" VT_RST, time);
    // clang-format on
    // "check time"
    ...
    if (this->type != 5) {
        if (((gSaveContext.zeldaTime < 0xB888) || (!gSaveContext.nightFlag)) && !(gSaveContext.memory.information.eventChkInf[8] & 1)) {
            this->actionFunc = EnHeishi1_SetupWalk;
        } else {
            Actor_Kill(&this->actor);
        }
    } else {
        if ((gSaveContext.zeldaTime >= 0xB889) || (gSaveContext.nightFlag) || (gSaveContext.memory.information.eventChkInf[8] & 1)) {
