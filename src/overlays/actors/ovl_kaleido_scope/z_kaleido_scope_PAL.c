...
void KaleidoScope_Draw(GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_kaleido_scope_PAL.c", 3188);

    pauseCtx->inputX = input->rel.x;
    pauseCtx->inputY = input->rel.y;

    gSPSegment(NEXT_DISP, 0x02, interfaceCtx->parameterSegment);
    gSPSegment(NEXT_DISP, 0x07, pauseCtx->unk_13C);
    gSPSegment(NEXT_DISP, 0x08, pauseCtx->unk_128);
    gSPSegment(NEXT_DISP, 0x09, pauseCtx->unk_12C);
    gSPSegment(NEXT_DISP, 0x0A, pauseCtx->unk_138);
    gSPSegment(NEXT_DISP, 0x0C, pauseCtx->unk_130);
    gSPSegment(NEXT_DISP, 0x0D, pauseCtx->unk_134);
    ...
    if (pauseCtx->flag == 0) {
        ...
        gDPSetCombineLERP(NEXT_DISP, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0,
    ...
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_kaleido_scope_PAL.c", 3254);
}
