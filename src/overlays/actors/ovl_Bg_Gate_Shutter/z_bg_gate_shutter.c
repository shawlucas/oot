...
void BgGateShutter_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (((gSaveContext.memory.information.infTable[7] & 0x40) || (gSaveContext.memory.information.eventChkInf[4] & 0x20)) &&
...
void func_8087828C(BgGateShutter* this, GlobalContext* globalCtx) {
    if (this->openingState == 1 && !(gSaveContext.memory.information.infTable[7] & 0x40)) {
        this->unk_178 = 2;
...
void BgGateShutter_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_gate_shutter.c", 323);
    func_80093D18(globalCtx->state.gfxCtx);

    gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_gate_shutter.c", 328),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_DISP, D_06001CD0);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_gate_shutter.c", 333);
}
