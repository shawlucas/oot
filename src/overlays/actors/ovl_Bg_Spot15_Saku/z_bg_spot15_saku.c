...
void BgSpot15Saku_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((gSaveContext.memory.information.infTable[7] & 2) != 0) {
...
void func_808B4930(BgSpot15Saku* this, GlobalContext* globalCtx) {
    if (this->unk_168 && !(gSaveContext.memory.information.infTable[7] & 2)) {
        this->unk_17C = 2;
...
void BgSpot15Saku_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_spot15_saku.c", 259);
    func_80093D84(globalCtx->state.gfxCtx);

    gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot15_saku.c", 263),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_POLY_XLU_DISP, D_060003C0);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_spot15_saku.c", 268);
}
