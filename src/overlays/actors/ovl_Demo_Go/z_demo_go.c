...
void func_8097C8A8(DemoGo* this, GlobalContext* globalCtx) {
    ...
    if ((thisx->params == 0) || (thisx->params == 1)) {
        SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->projectionMatrix, &thisx->posRot.pos, &sp20, &sp1C);
...
void func_8097D29C(DemoGo* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_demo_go.c", 732);

    func_80093D18(globalCtx->state.gfxCtx);
    gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(srcSegment8));
    gSPSegment(NEXT_DISP, 0x09, SEGMENTED_TO_VIRTUAL(srcSegment9));

    SkelAnime_DrawSV(globalCtx, skelAnime->skeleton, skelAnime->limbDrawTbl, skelAnime->dListCount, NULL, NULL,
                     &this->actor);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_demo_go.c", 746);
}
