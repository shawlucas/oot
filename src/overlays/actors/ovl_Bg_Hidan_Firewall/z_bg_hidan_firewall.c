...
void BgHidanFirewall_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_hidan_firewall.c", 448);
    SET_NOW_POLY_XLU_DISP(Gfx_CallSetupDL(NOW_POLY_XLU_DISP, 20));

    gSPSegment(NEXT_POLY_XLU_DISP, 0x08, SEGMENTED_TO_VIRTUAL(D_80886D04[this->unk_150]));
    gDPSetPrimColor(NEXT_POLY_XLU_DISP, 0, 0x01, 255, 255, 0, 150);
    gDPSetEnvColor(NEXT_POLY_XLU_DISP, 255, 0, 0, 255);
    gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_hidan_firewall.c", 458),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(NEXT_POLY_XLU_DISP, D_0600DA80);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_hidan_firewall.c", 463);
}
