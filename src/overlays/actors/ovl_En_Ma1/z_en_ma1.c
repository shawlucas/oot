...
u16 EnMa1_GetText(GlobalContext* globalCtx, Actor* thisx) {
    ...
    if (gSaveContext.memory.information.eventChkInf[1] & 0x40) {
        return 0x2049;
    }
    if (gSaveContext.memory.information.eventChkInf[1] & 0x20) {
        if ((gSaveContext.memory.information.infTable[8] & 0x20)) {
            return 0x2049;
        } else {
            return 0x2048;
        }
    }
    if (gSaveContext.memory.information.eventChkInf[1] & 0x10) {
        return 0x2047;
    }
    if (gSaveContext.memory.information.eventChkInf[1] & 4) {
        return 0x2044;
    }
    if (gSaveContext.memory.information.infTable[8] & 0x10) {
        if (gSaveContext.memory.information.infTable[8] & 0x800) {
            return 0x2043;
        } else {
            return 0x2042;
        }
...
s16 func_80AA0778(GlobalContext* globalCtx, Actor* thisx) {
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        case 2:
            switch (thisx->textId) {
                case 0x2041:
                    gSaveContext.memory.information.infTable[8] |= 0x10;
                    gSaveContext.memory.information.eventChkInf[1] |= 1;
                    ret = 0;
                    break;
                case 0x2043:
                    ret = 1;
                    break;
                case 0x2047:
                    gSaveContext.memory.information.eventChkInf[1] |= 0x20;
                    ret = 0;
                    break;
                case 0x2048:
                    gSaveContext.memory.information.infTable[8] |= 0x20;
                    ret = 0;
                    break;
                case 0x2049:
                    gSaveContext.memory.information.eventChkInf[1] |= 0x40;
                    ret = 0;
                    break;
                case 0x2061:
                    ret = 2;
...
s32 func_80AA08C4(EnMa1* this, GlobalContext* globalCtx) {
    if ((this->actor.shape.rot.z == 3) && (gSaveContext.gameInfo.sceneSetupIndex == 5)) {
        return 1;
    }
    ...
    if (((globalCtx->sceneNum == SCENE_MARKET_NIGHT) || (globalCtx->sceneNum == SCENE_MARKET_DAY)) &&
        (!(gSaveContext.memory.information.eventChkInf[1] & 0x10)) && (!(gSaveContext.memory.information.infTable[8] & 0x800))) {
        return 1;
    }
    if ((globalCtx->sceneNum == SCENE_SPOT15) && (!(gSaveContext.memory.information.eventChkInf[1] & 0x10))) {
        if (gSaveContext.memory.information.infTable[8] & 0x800) {
            return 1;
        } else {
            gSaveContext.memory.information.infTable[8] |= 0x800;
            return 0;
        }
    }
    if ((globalCtx->sceneNum == SCENE_SOUKO) && (gSaveContext.nightFlag == 1) && (gSaveContext.memory.information.eventChkInf[1] & 0x10)) {
        return 1;
    }
    if (globalCtx->sceneNum != SCENE_SPOT20) {
        return 0;
    }
    if ((this->actor.shape.rot.z == 3) && (gSaveContext.nightFlag == 0) && (gSaveContext.memory.information.eventChkInf[1] & 0x10)) {
        return 1;
    }
    return 0;
}
...
void EnMa1_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((!(gSaveContext.memory.information.eventChkInf[1] & 0x10)) || (CHECK_QUEST_ITEM(QUEST_SONG_EPONA))) {
...
void func_80AA0D88(EnMa1* this, GlobalContext* globalCtx) {
    ...
    if ((globalCtx->sceneNum == SCENE_SPOT15) && (gSaveContext.memory.information.eventChkInf[1] & 0x10)) {
        Actor_Kill(&this->actor);
    } else if ((!(gSaveContext.memory.information.eventChkInf[1] & 0x10)) || (CHECK_QUEST_ITEM(QUEST_SONG_EPONA))) {
...
void func_80AA0EFC(EnMa1* this, GlobalContext* globalCtx) {
    if (this->unk_1E8.unk_00 == 3) {
        this->unk_1E8.unk_00 = 0;
        this->actionFunc = func_80AA0D88;
        gSaveContext.memory.information.eventChkInf[1] |= 4;
        globalCtx->msgCtx.msgMode = 0x36;
    }
}
...
void EnMa1_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_ma1.c", 1226);
    camera = ACTIVE_CAM;
    someFloat = Math_Vec3f_DistXZ(&this->actor.posRot.pos, &camera->eye);
    func_800F6268(someFloat, 0x2F);
    func_80093D18(globalCtx->state.gfxCtx);

    gSPSegment(NEXT_DISP, 0x09, SEGMENTED_TO_VIRTUAL(D_80AA16C4[this->unk_1E6]));
    gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(D_80AA16D0[this->unk_1E4]));

    SkelAnime_DrawSV(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl, this->skelAnime.dListCount,
                     EnMa1_OverrideLimbDraw, EnMa1_PostLimbDraw, &this->actor);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_ma1.c", 1261);
}
