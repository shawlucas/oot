...
void BgSpot07Taki_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_bg_spot07_taki.c", __LINE__);
    frames = globalCtx->gameplayFrames;
    if (LINK_IS_ADULT) {
        func_80093D18(globalCtx->state.gfxCtx);
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot07_taki.c", __LINE__),
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot07_taki.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_bg_spot07_taki.c", __LINE__);
}
