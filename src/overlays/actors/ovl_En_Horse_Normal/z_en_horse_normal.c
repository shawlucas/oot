...
void EnHorseNormal_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_horse_normal.c", __LINE__);
    ...
    if (this->action == HORSE_WAIT_CLONE) {
        ...
        mtx2 = Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_horse_normal.c", __LINE__);
        if (mtx2 != NULL) {
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_horse_normal.c", __LINE__);
}
