...
void DemoKankyo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (objBankIndex < 0) {
        ASSERT(0, "0", "../z_demo_kankyo.c", __LINE__);
    } else {
...
void DemoKankyo_DrawRain(Actor* thisx, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 30; i++) {
        ...
        for (j = 0; j < 5; j++) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_kankyo.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            POLY_XLU_DISP = Gfx_CallSetupDL(POLY_XLU_DISP, 0x14);
            gSPDisplayList(POLY_XLU_DISP++, object_efc_star_field_DL_000080);
        }
    }
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_demo_kankyo.c", __LINE__);
}
...
void DemoKankyo_DrawRock(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_kankyo.c", __LINE__),
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
