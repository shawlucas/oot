...
void EnMa3_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_ma3.c", 927);

    if (limbIndex == 18) {
        Matrix_MultVec3f(&vec, &thisx->posRot2.pos);
    }
    if ((limbIndex == 14) && (this->skelAnime.animCurrentSeg == &D_060093BC)) {
        gSPDisplayList(NEXT_DISP, &D_06005420);
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_ma3.c", 950);
}
...
void EnMa3_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_ma3.c", 978);
    ...
    gSPSegment(NEXT_DISP, 0x09, SEGMENTED_TO_VIRTUAL(D_80AA38A4[this->unk_210]));
    gSPSegment(NEXT_DISP, 0x08, SEGMENTED_TO_VIRTUAL(D_80AA38B0[this->unk_20E]));

    SkelAnime_DrawSV(globalCtx, this->skelAnime.skeleton, this->skelAnime.limbDrawTbl, this->skelAnime.dListCount,
                     EnMa3_OverrideLimbDraw, EnMa3_PostLimbDraw, &this->actor);
    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_ma3.c", 1013);
}
