...
void func_808B7478(Actor* thisx, GameState* state);
...
void func_808B7478(Actor* thisx, GameState* state) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_bg_spot17_funen.c", 153);
    func_80093D84(globalCtx->state.gfxCtx);
    Matrix_RotateY((s16)(func_8005A9F4(ACTIVE_CAM) - thisx->shape.rot.y + 0x8000) * 9.58738019108e-05f, MTXMODE_APPLY);

    gSPMatrix(NEXT_POLY_XLU_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_bg_spot17_funen.c", 161),
    ...
    gSPDisplayList(NEXT_POLY_XLU_DISP, D_06000B40);

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_bg_spot17_funen.c", 176);
}
