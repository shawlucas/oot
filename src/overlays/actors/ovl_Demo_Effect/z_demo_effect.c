...
void DemoEffect_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (objectIndex < 0) {
        ASSERT(0, "0", "../z_demo_effect.c", __LINE__);
    } else {
...
void DemoEffect_DrawJewel(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (!DemoEffect_CheckCsAction(this, globalCtx, 1)) {
        ...
        if (!(this->effectFlags & 0x1)) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_demo_effect.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_demo_effect.c", __LINE__);
}
...
void DemoEffect_DrawTimeWarp(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (effectType == DEMO_EFFECT_TIMEWARP_TIMEBLOCK_LARGE || effectType == DEMO_EFFECT_TIMEWARP_TIMEBLOCK_SMALL ||
        ...
        OPEN_DISPS(gfxCtx, "../z_demo_effect.c", __LINE__);
        ...
        CLOSE_DISPS(gfxCtx, "../z_demo_effect.c", __LINE__);
    }
}
