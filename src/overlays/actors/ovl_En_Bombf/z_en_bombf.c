...
Gfx* EnBombf_NewMtxDList(GraphicsContext* gfxCtx, GlobalContext* globalCtx) {
    ...
    gSPMatrix(displayListHead++, Matrix_NewMtx(gfxCtx, "../z_en_bombf.c", __LINE__),
...
void EnBombf_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params <= BOMBFLOWER_BODY) {
        ...
        if (thisx->params != BOMBFLOWER_BODY) {
            gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_bombf.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_bombf.c", __LINE__);
}
