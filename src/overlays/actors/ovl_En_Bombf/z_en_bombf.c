    ...
    if (thisx->params == BOMBFLOWER_BODY) {
        ...
        if (this->unk_200 != 0) {
            ...
            if (this->timer == 0) {
                ...
                func_8005AA1C(&globalCtx->camera, 2, 0xB, 8);
...
void EnBombf_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    OPEN_DISP(globalCtx->state.gfxCtx, "../z_en_bombf.c", 1034);

    if (thisx->params <= BOMBFLOWER_BODY) {
        func_80093D18(globalCtx->state.gfxCtx);

        if (thisx->params != BOMBFLOWER_BODY) {
            gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_bombf.c", 1041),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(NEXT_DISP, D_06000340);
            gSPDisplayList(NEXT_DISP, D_06000530);

            Matrix_Translate(0.0f, 1000.0f, 0.0f, MTXMODE_APPLY);
            Matrix_Scale(this->flowerBombScale, this->flowerBombScale, this->flowerBombScale, MTXMODE_APPLY);
        }

        gDPSetPrimColor(NEXT_DISP, 0, 0, 200, 255, 200, 255);
        gDPPipeSync(NEXT_DISP);
        gDPSetEnvColor(NEXT_DISP, (s16)this->flashIntensity, 20, 10, 0);
        gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_en_bombf.c", 1054),
                  G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPSegment(NEXT_DISP, 0x08,
                   SEGMENTED_TO_VIRTUAL(EnBombf_NewMtxDList(globalCtx->state.gfxCtx, globalCtx)));
        gSPDisplayList(NEXT_DISP, D_06000408);
    } else {
        func_800628A4(0, &this->explosionCollider);
    }

    CLOSE_DISP(globalCtx->state.gfxCtx, "../z_en_bombf.c", 1063);
}
