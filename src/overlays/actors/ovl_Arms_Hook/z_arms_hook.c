...
void ArmsHook_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (player->actor.draw != NULL) {
        if (player->unk_15D == 0xF) {
            OPEN_DISP(globalCtx->state.gfxCtx, "../z_arms_hook.c", 850);
            ...
            gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arms_hook.c", 895),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(NEXT_DISP, D_0602B288);
            ...
            gSPMatrix(NEXT_DISP, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arms_hook.c", 910),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(NEXT_DISP, D_0602AFF0);

            CLOSE_DISP(globalCtx->state.gfxCtx, "../z_arms_hook.c", 913);
        }
    }
}
