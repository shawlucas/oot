...
void ArmsHook_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((player->actor.draw != NULL) && (player->rightHandType == 15)) {
        OPEN_DISPS(globalCtx->state.gfxCtx, "../z_arms_hook.c", __LINE__);
        ...
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_arms_hook.c", __LINE__),
        ...
        CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_arms_hook.c", __LINE__);
    }
}
