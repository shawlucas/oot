...
block_1:
    ...
    if (*dList != NULL) {
        ...
        gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_boss_dodongo.c", __LINE__),
...
void BossDodongo_DrawEffects(GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(gfxCtx, "../z_boss_dodongo.c", __LINE__);
    ...
    for (i = 0; i < 80; i++, eff++) {
        if (eff->unk_24 == 1) {
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx, "../z_boss_dodongo.c", __LINE__),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, object_kingdodongo_DL_009DD0);
        }
    }

    CLOSE_DISPS(gfxCtx, "../z_boss_dodongo.c", __LINE__);
}
