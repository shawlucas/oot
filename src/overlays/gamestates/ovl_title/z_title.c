...
void Title_Draw(TitleContext* this) {
    ...
    OPEN_DISP(this->state.gfxCtx, "../z_title.c", 395);
    ...
    gSPSetLights1(NEXT_DISP, sTitleLights);
    ...
    gSPMatrix(NEXT_DISP, Matrix_NewMtx(this->state.gfxCtx, "../z_title.c", 424), G_MTX_LOAD);
    gSPDisplayList(NEXT_DISP, &D_01002720);
    func_800944C4(this->state.gfxCtx);
    gDPPipeSync(NEXT_DISP);
    gDPSetCycleType(NEXT_DISP, G_CYC_2CYCLE);
    gDPSetRenderMode(NEXT_DISP, G_RM_XLU_SURF2, G_RM_OPA_CI | CVG_DST_WRAP);
    gDPSetCombineLERP(NEXT_DISP, TEXEL1, PRIMITIVE, ENV_ALPHA, TEXEL0, 0, 0, 0, TEXEL0, PRIMITIVE,
                      ENVIRONMENT, COMBINED, ENVIRONMENT, COMBINED, 0, PRIMITIVE, 0);
    gDPSetPrimColor(NEXT_DISP, 0, 0, 170, 255, 255, 255);
    gDPSetEnvColor(NEXT_DISP, 0, 0, 255, 128);

    gDPLoadMultiBlock(NEXT_DISP, &D_01001800, 0x100, 1, G_IM_FMT_I, G_IM_SIZ_8b, 32, 32, 0,
                      G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 5, 5, 2, 11);

    for (idx = 0, y = 94; idx < 16; idx++, y += 2) {
        gDPLoadTextureBlock(NEXT_DISP, &D_01000000[0x180 * idx], G_IM_FMT_I, G_IM_SIZ_8b, 192, 2, 0,
                            G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
                            G_TX_NOLOD);

        gDPSetTileSize(NEXT_DISP, 1, this->uls, (this->ult & 0x7F) - idx * 4, 0, 0);
        gSPTextureRectangle(NEXT_DISP, 388, y << 2, 1156, (y + 2) << 2, G_TX_RENDERTILE, 0, 0, 1024, 1024);
    }
    ...
    CLOSE_DISP(this->state.gfxCtx, "../z_title.c", 483);
}

void Title_Main(GameState* thisx) {
    TitleContext* this = (TitleContext *)thisx;

    OPEN_DISP(this->state.gfxCtx, "../z_title.c", 494);

    gSPSegment(NEXT_DISP, 0, NULL);
    gSPSegment(NEXT_DISP, 1, this->staticSegment);
    ...
    CLOSE_DISP(this->state.gfxCtx, "../z_title.c", 541);
}
