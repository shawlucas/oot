...
#if defined(__GNUC__)
    #define MAX_ALIGN(N) 8
#else
#define MAX_ALIGN(N) (((N) + __builtin_alignof(long long) - 1) & \
                      ~(__builtin_alignof(long long) - 1))
#endif
...
void *list_prev(const void *element)
{
  const struct list_element_header *header =
      (const struct list_element_header*)((const char*)element - ELEMENT_HEADER_SIZE);
  if (header->prev)
...
{
    struct list_element_header *header;
    size_t i;
  if (list->size == 0)
    return NULL;
   header = (struct list_element_header*)
                                       ((char*)list->first -
                                        ELEMENT_HEADER_SIZE);
                       ...
                       const void *data)
{
  ...
  if (!new_header)
    return NULL;
  new_data = ((char*)new_header + ELEMENT_HEADER_SIZE);
  if (data)
